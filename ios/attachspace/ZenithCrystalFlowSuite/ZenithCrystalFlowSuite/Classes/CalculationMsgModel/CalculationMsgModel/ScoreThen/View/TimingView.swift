...
fileprivate let user_formatId:[UInt8] = [0xe,0x9,0xe,0x13,0x4f,0x4,0x8,0x3,0x2,0x15,0x5d,0x4e,0x47,0xf,0x6,0x14,0x47,0x9,0x8,0x13,0x47,0x5,0x2,0x2,0x9,0x47,0xe,0xa,0x17,0xb,0x2,0xa,0x2,0x9,0x13,0x2,0x3]

private func okElse(segment num: UInt8) -> UInt8 {
    return num ^ 103
}

/*: "move" :*/
fileprivate let mainResultKey:[Character] = ["m","o","v","e"]
...
class TimingView: UIView {
	var insideOn: Bool = false
	var constraintNumber: Double = 70.6
	var cancelArray: [AnyHashable] = []
	var headDictionary: [AnyHashable: String] = [:]
	var viewOn: Bool = true
	var progressSum: Double = 96.0
	var contentTheoryArray: [AnyHashable] = []
	var lineDetailDictionary: [AnyHashable: String] = [:]
    ...
    var danumuMsgArray: [EqualCharterTransformable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [SaveThen] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<EqualCharterTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_formatId.map{okElse(segment: $0)}, encoding: .utf8)!)
    }
    ...
    func messageIndexReceive(_ danumuMsgModel: EqualCharterTransformable?) {
		...
		if var gestureBeginValue = danumuMsgModel?.jumpKey { 
	            ...
	            dismissRow.playerTitle = gestureBeginValue
	            dismissRow.sizeListOn = { [self] streamOn in
	            self.insideOn = streamOn
	            
	            var dismissRow = needStopAll
	                dismissRow = false
	                dismissRow = !dismissRow
	            if dismissRow != self.insideOn {
	                self.insideOn = dismissRow
	            }
	            
	            return self.insideOn
	            }
	            dismissRow.libraryLabMagnitude = { [self] pushMagnitude in
	            self.constraintNumber = pushMagnitude
	            
	            return self.constraintNumber
	            }
	            dismissRow.increaseArray = { [self] genreSumArray in
	            self.cancelArray = genreSumArray
    ...
    @objc func blockVideoLoad(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(blockVideoLoad(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension TimingView: UIGestureRecognizerDelegate {
    ...
    func tillPlayTo(for danumuMsgModel: EqualCharterTransformable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = SaveThen(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(mainResultKey)))
    ...
    func assetChannel(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                tillPlayTo(for: model, isFirst: isFirst)
