...
/*: s" :*/
fileprivate let userRequestMicUrl:[Character] = ["s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let dream_pricePathTitle:[UInt8] = [0xf5,0xf2,0xf5,0xe8,0xb4,0xff,0xf3,0xf8,0xf9,0xee,0xa6,0xb5,0xbc,0xf4,0xfd,0xef,0xbc,0xf2,0xf3,0xe8,0xbc,0xfe,0xf9,0xf9,0xf2,0xbc,0xf5,0xf1,0xec,0xf0,0xf9,0xf1,0xf9,0xf2,0xe8,0xf9,0xf8]

/*: "bth_live_follow_pre" :*/
fileprivate let k_intimateUrl:String = "bth_lifalse container"
fileprivate let show_upKey:[Character] = ["v","e","_","f","o"]
fileprivate let m_clickStr:String = "llow_preframe model to section"

/*: "btn_live_mini_nor" :*/
fileprivate let app_giftText:String = "icon lab framebtn_li"
fileprivate let main_atText:String = "ini_norfalse range return self action"

/*: "#2DF2FF" :*/
fileprivate let show_sourceContent:String = "layer count if#2DF2F"
fileprivate let noti_equalMsg:String = "share"

/*: "Live" :*/
fileprivate let noti_cellPath:[Character] = ["L","i","v","e"]

/*: "#FF506D" :*/
fileprivate let main_textData:String = "#FF506Dto share"

/*: "  :*/
fileprivate let m_permissionMsg:String = " "

/*: "removeAttentionUid" :*/
fileprivate let notiToolFormat:[Character] = ["r","e","m","o","v","e"]
fileprivate let mRowTabFormat:[Character] = ["A","t","t","e","n"]
fileprivate let userPlayerPath:String = "tionUidmanager self self max"

/*: "attentionUid" :*/
fileprivate let app_userMsg:[Character] = ["a","t","t","e","n","t","i","o"]
fileprivate let dreamJumpName:String = "nUidlet return actual pi user"

/*: "source" :*/
fileprivate let k_appearKey:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let noti_pathContent:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let mResultCellIdent:[Character] = ["U","I","D",":"]

/*: "btn_live_closed_nor" :*/
fileprivate let userRootText:[Character] = ["b","t","n","_","l","i","v","e"]
fileprivate let main_userUrl:[Character] = ["_","c","l","o","s","e"]
fileprivate let appActionName:String = "d_norkind path push bind"
...
class WithTopView: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: ListRecognizerDelegate?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= PathAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                self.countdownLab.text = "\(m_videoCellScreenPath - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.buttonReceive(countdown: "\(m_videoCellScreenPath - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = ViewModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_pricePathTitle.map{$0^156}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(scratch), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        lab.font = UIFont.upperClassMove(fontSize: 11)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(k_intimateUrl.prefix(6)) + String(show_upKey) + String(m_clickStr.prefix(8)))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(modelWill), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(app_giftText.suffix(6)) + "ve_m" + String(main_atText.prefix(7)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(fixingGesture), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: TelevisionButton = {
        ...
        let image = UIImage.giftColor(color: UIColor(hex: (String(show_sourceContent.suffix(6)) + noti_equalMsg.replacingOccurrences(of: "share", with: "F")))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.click(), for: .normal)
        img.setImage(image.click(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(noti_cellPath)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(main_textData.prefix(7))))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(PathAppManager.share.appConfigMode.countdown2)s"
        lab.text = " \(PathAppManager.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: LiveCustomView = {
        //: let v = TalkingLiveCountdownView()
        let v = LiveCustomView()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.valuePlayCan(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        //: btn.addTarget(self, action: #selector(onlineNumBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(subBy), for: .touchUpInside)
        //: btn.layer.cornerRadius = actualWidth(w: 32)/2
        btn.layer.cornerRadius = actualWidth(w: 32) / 2
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 12)
        btn.titleLabel?.font = UIFont.upperClassMove(fontSize: 12)
        //: return btn
        return btn
        //: }()
    }()
...
extension WithTopView {
    ...
    @objc func countModel(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(notiToolFormat) + String(mRowTabFormat) + String(userPlayerPath.prefix(7)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(app_userMsg) + String(dreamJumpName.prefix(4)))] = uid
            //: dict["source"] = "1"
            dict[(String(k_appearKey))] = "1"
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        SourceRequestTool.alongTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension WithTopView {
    ...
    @objc private func subBy() {
        ...
        self.onlineListView.doingCurrent(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func onLineTo() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        WithThen().campAndCompletion(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(String(noti_pathContent))].string
    ...
    private func marque() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(onLineTo), userInfo: nil, repeats: true)
    }
...
extension WithTopView {
    ...
    func addTo(_ liveModel: ViewModelType) {
        ...
        uidLab.text = (String(mResultCellIdent)) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.sumercalate(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.indicatorSex(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            ...
            miniBtn.setImage(UIImage.sendCollection(name: (String(app_giftText.suffix(6)) + "ve_m" + String(main_atText.prefix(7)))), for: .normal)
            //: refreshFollow(isAttention: true)
            isAdd(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.BundleImageNamed(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.sendCollection(name: (String(userRootText) + String(main_userUrl) + String(appActionName.prefix(5)))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            isAdd(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }

    /// 最小化/关闭直播 按钮点击事件
    //: @objc func miniButtonClick() {
    @objc func fixingGesture() {
        //: rightBtnBlock?()
        rightBtnBlock?()
        // 是否是主播
        //: let isAnchor = (String(liveModel.streamerInfo.uid) == PathAppManager.share.loginUid)
        ...
        if isAnchor { // 改为小窗口
            ...
            CommentLiveManager.leaveBy().tentSuccessTransom(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            withTo()?.navigationController?.popViewController(animated: false)
        }
    }
    ...
    @objc private func scratch() {
        ...
        createEqual(uid: String(CommentLiveManager.leaveBy().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func createEqual(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = ManagerReactiveCompatible(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any ViewDelegate
        //: userCard.show()
        userCard.bigFrontTo()
    }
    ...
    private func isAdd(isAttention: Bool) {
    ...
    private func campOut() {
        //: self.liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        self.liveStatusImgV.isHidden = !CommentLiveManager.leaveBy().isLive
    ...
    @objc private func modelWill() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        countModel(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.isAdd(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension WithTopView {
    ...
    private func textDetail() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: app_screenFailMarginFormat, height: app_topMsg + actualWidth(w: 68))
