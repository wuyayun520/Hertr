...
fileprivate let noti_imageText:[UInt8] = [0xb2,0xb5,0xb2,0xaf,0xf3,0xb8,0xb4,0xbf,0xbe,0xa9,0xe1,0xf2,0xfb,0xb3,0xba,0xa8,0xfb,0xb5,0xb4,0xaf,0xfb,0xb9,0xbe,0xbe,0xb5,0xfb,0xb2,0xb6,0xab,0xb7,0xbe,0xb6,0xbe,0xb5,0xaf,0xbe,0xbf]

private func viewModel(instance num: UInt8) -> UInt8 {
    return num ^ 219
}

/*: "nav_back_black_nor" :*/
fileprivate let main_picStr:String = "i let block centernav_ba"
fileprivate let showLocationIdent:[Character] = ["k","_","n","o","r"]

/*: "btn_detail_more" :*/
fileprivate let dreamTitleFormat:String = "btn_style style let touch"
fileprivate let show_managerName:String = "l_moreview max"

/*: "btn_chat_videocall" :*/
fileprivate let app_textMiniPath:String = "user type data varbtn_chat"
fileprivate let dream_errorFormatKey:String = "add let var command close_vide"

/*: "icon_lounge" :*/
fileprivate let user_componentMessage:String = "make for playicon_l"

/*: "Official" :*/
fileprivate let userPositionMessage:[Character] = ["O","f","f","i","c","i","a","l"]

/*: "Online List" :*/
fileprivate let showGiftMessage:[Character] = ["O","n","l","i","n","e"," ","L","i","s","t"]

/*: "icon_privateChat_intimate" :*/
fileprivate let showSuiteName:[Character] = ["i","c","o","n","_","p","r","i","v","a","t","e","C"]
fileprivate let m_mortalFaceLabelMessage:String = "H"
fileprivate let userColorStr:String = "lab in view rightat_i"

/*: "#FF60B9" :*/
fileprivate let user_viewName:[Character] = ["#","F","F","6","0","B","9"]

/*: "Customer Care Center" :*/
fileprivate let m_modeStr:String = "Custoon empty"
fileprivate let showMomentId:String = "view self addCare Cen"
fileprivate let m_equalMsg:[Character] = ["t","e","r"]

/*: "#48FF1E" :*/
fileprivate let showRecordId:String = "#48FF1Elength self"

/*: "Online" :*/
fileprivate let user_imageMessage:[Character] = ["O","n","l","i","n","e"]

/*: "#FF4B7F" :*/
fileprivate let main_frameContent:[Character] = ["#","F","F","4","B","7","F"]

/*: "busy" :*/
fileprivate let main_exitUrl:[Character] = ["b","u","s","y"]

/*: "+ :*/
fileprivate let app_stopText:String = "+"

/*: "get json error" :*/
fileprivate let user_listenTitle:[Character] = ["g","e","t"," ","j","s","o","n"]
fileprivate let kEffectListData:String = " errorto if var icon self"

/*: "transform.rotation" :*/
fileprivate let userTagPath:[Character] = ["t","r","a","n","s","f","o","r","m",".","r","o","t","a"]
fileprivate let mainFormatLayerIdent:[Character] = ["t","i","o","n"]

/*: "transform.scale" :*/
fileprivate let mainNoTitle:String = "component"
fileprivate let show_sValue:String = "makeansfo"

/*: "zoom&shake" :*/
fileprivate let mainManagerMessage:String = "zoom&svar detail"
...
// __CLOSE_PRINT__
//
//  FinancialAidReactiveCompatible.swift
//  AbroadTalking
//
...
class FinancialAidReactiveCompatible: UIView {
    //: private let disposeBag = DisposeBag()
    private let disposeBag = DisposeBag()
    //: private var chatType: DataIMChatType = .private
    private var chatType: DataIMChatType = .private
    //: private var userId = ""
    private var userId = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_imageText.map{viewModel(instance: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var backButton: UIButton = {
        ...
        let img = UIImage.sendCollection(name: (String(main_picStr.suffix(6)) + "ck_blac" + String(showLocationIdent))).withRenderingMode(.alwaysTemplate)
    ...
    lazy var moreButton: UIButton = {
        ...
        let img = UIImage.sendCollection(name: (String(dreamTitleFormat.prefix(4)) + "detai" + String(show_managerName.prefix(6)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var videoCallBtn: UIButton = {
        ...
        let img = UIImage.sendCollection(name: (String(app_textMiniPath.suffix(8)) + String(dream_errorFormatKey.suffix(5)) + "ocall")).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (String(user_componentMessage.suffix(6)) + "ounge"))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var officalImgV: UIButton = {
        ...
        img.valuePlayCan(color: UIColor.semblance(), forState: .normal)
        //: img.setTitle("Official".localized, for: .normal)
        img.setTitle((String(userPositionMessage)).localized, for: .normal)
        //: img.setTitleColor(.white, for: .normal)
        img.setTitleColor(.white, for: .normal)
        //: img.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 11)
        img.titleLabel?.font = UIFont.indicatorSize(fontSize: 11)
    ...
    lazy var onlineListBtn: UIButton = {
        ...
        btn.setTitle((String(showGiftMessage)).localized, for: .normal)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineStatusV: TelevisionButton = {
    ...
    private lazy var myBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(speed), for: .touchUpInside)
    ...
    private lazy var intimateBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(showSuiteName) + m_mortalFaceLabelMessage.lowercased() + String(userColorStr.suffix(4)) + "ntimate")), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(sendWrite), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var intimateLB: UILabel = {
        ...
        lb.font = UIFont.upperClassMove(fontSize: 12)
        //: lb.isHidden = true
        lb.isHidden = true
        //: return lb
        return lb
    ...
    private lazy var addIntimateLB: UILabel = {
        ...
        lb.font = UIFont.bindFor(type: .Semibold, fontSize: 12)
        //: lb.isHidden = true
        lb.isHidden = true
...
extension FinancialAidReactiveCompatible {
    //: func createSubViews() {
    func nameForViews() {
        ...
        if self.chatType == .service {
            ...
            titleLabel.text = (String(m_modeStr.prefix(5)) + "mer " + String(showMomentId.suffix(8)) + String(m_equalMsg)).localized
            //: titleLabel.font = .pingfangFont(type: .Medium, fontSize: 18)
            titleLabel.font = .bindFor(type: .Medium, fontSize: 18)
            //: titleLabel.textColor = .appTitleColor()
            titleLabel.textColor = .statusBy()
    ...
    func tableRow() {
        ...
        if self.chatType == .service { // 官方客服
            ...
            let offset = PresentReactiveCompatible.shared.direction == .leftToRight ? 22 : -22
    ...
    ///   - userModel: 用户信息
    ///   - isFrom: 列表控制器入口
    //: func private_updateNavView(userModel: VisualCommunicationReactiveCompatible, isFrom: PrivateChatIsFromEnum) {
    func metadataName(userModel: VisualCommunicationReactiveCompatible, isFrom: ImageSharedFromEnum) {
        ...
        if userModel.onlineStatus == 1 {
            ...
            let image = UIImage.giftColor(color: UIColor(hex: (String(showRecordId.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.click(), for: .normal)
            onlineStatusV.setImage(image.click(), for: .normal)
            //: onlineStatusV.setTitle("Online".localized, for: .normal)
            onlineStatusV.setTitle((String(user_imageMessage)).localized, for: .normal)
            ...
            let image = UIImage.giftColor(color: UIColor(hex: (String(main_frameContent)))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.click(), for: .normal)
            onlineStatusV.setImage(image.click(), for: .normal)
            //: onlineStatusV.setTitle("busy".localized, for: .normal)
            onlineStatusV.setTitle((String(main_exitUrl)).localized, for: .normal)
            //: } else {
        } else {
        ...
        titleLabel.textColor = userModel.loungePlus ? .needEnable() : .white
        ...
        var videoCall = userModel.enableVideoCall
        //: if isFrom == .LiveRoom || PathAppManager.share.appStatus == AppSkinStatus.special.rawValue {
        if isFrom == .LiveRoom || PathAppManager.share.appStatus == AssemblageHashable.special.rawValue {
            //: videoCall = false
            videoCall = false
        }
        //: if videoCall {
    ...
    /// - Parameters:
    ///   - userModel: 用户信息
    //: func private_updateIntimateNavView(userModel: VisualCommunicationReactiveCompatible) {
    func rootModel(userModel: VisualCommunicationReactiveCompatible) {
        ...
        userBtn.isHidden = false
        //: myBtn.setUrlImage(urlStr: PathAppManager.share.loginUserMode.headPic ?? "")
        myBtn.lean(urlStr: PathAppManager.share.loginUserMode.headPic ?? "")
        //: userBtn.setUrlImage(urlStr: userModel.headPic)
        userBtn.lean(urlStr: userModel.headPic)
    ...
    func finishDisplay(intimate: Int) {
        //: let str  = "+\(intimate)"
        let str = "+\(intimate)"
        ...
        let attributes = [.foregroundColor: UIColor.white, .font: UIFont.bindFor(type: .Semibold, fontSize: 12)] as [NSAttributedString.Key: Any]
        ...
        let url = ReverberationInsideHoeHandleReactiveCompatible.default.pullInConfirmType(type: .Private_Chat_intimate)
        //: do {
        do {
            ...
            printLog(message: (String(user_listenTitle) + String(kEffectListData.prefix(6))))
        }
    }
    ...
    func objectCountimate(intimate: Int) {
    ...
    @objc func speed() {
        //: NamePushManager.share.func__pushToUserDetailVC(uid: PathAppManager.share.loginUserMode.userID)
        NamePushManager.share.pressAway(uid: PathAppManager.share.loginUserMode.userID)
    }

    //: @objc func userIconClick() {
    @objc func snapClick() {
        //: NamePushManager.share.func__pushToUserDetailVC(uid: userId)
        NamePushManager.share.pressAway(uid: userId)
    }
    ...
    @objc func sendWrite() {
        ...
        let view = EffectThen(frame: CGRect(x: 0, y: 0, width: app_screenFailMarginFormat, height: mainAcceptKey))
        //: view.show()
        view.giftShow()
    }
    ...
    func bridleAnimat() {
        ...
        let roate = CAKeyframeAnimation(keyPath: (String(userTagPath) + String(mainFormatLayerIdent)))
        ...
        let zoom = CAKeyframeAnimation(keyPath: (mainNoTitle.replacingOccurrences(of: "component", with: "t") + show_sValue.replacingOccurrences(of: "make", with: "r") + "rm.scale"))
        ...
        videoCallBtn.layer.add(groupAnnimation, forKey: (String(mainManagerMessage.prefix(6)) + "hake"))
    }
}
