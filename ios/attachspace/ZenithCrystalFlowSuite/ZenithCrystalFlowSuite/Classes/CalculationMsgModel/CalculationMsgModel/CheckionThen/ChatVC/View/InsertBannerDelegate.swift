...
fileprivate let m_toMsg:[UInt8] = [0xbd,0xba,0xbd,0xa0,0xfc,0xb7,0xbb,0xb0,0xb1,0xa6,0xee,0xfd,0xf4,0xbc,0xb5,0xa7,0xf4,0xba,0xbb,0xa0,0xf4,0xb6,0xb1,0xb1,0xba,0xf4,0xbd,0xb9,0xa4,0xb8,0xb1,0xb9,0xb1,0xba,0xa0,0xb1,0xb0]

/*: "icon_chats_banner_subtract" :*/
fileprivate let main_giftFormat:String = "icon_cself return equal title"
fileprivate let notiFormatName:String = "self label_bann"
fileprivate let mEqualMsg:String = "btraaspect"

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let dreamMakeValue:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e"]
fileprivate let k_modelUrl:[Character] = ["r","_","s","u","b","t"]
fileprivate let mLoadMsg:String = "execute shared data keyract_pre"

/*: "bannerList" :*/
fileprivate let dream_quoteMsg:String = "banncount"

/*: "JXBannerCellID" :*/
fileprivate let noti_equalId:String = "Jvalue"
fileprivate let m_interactionNamePath:[Character] = ["B","a","n","n","e","r","C","e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let kColorModelAsName:String = "#FFFFFequal match"
fileprivate let mainInputData:[Character] = ["F"]
...
protocol SmartBannerDelegate: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func gildTheLily(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class InsertBannerDelegate: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: SmartBannerDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_toMsg.map{$0^212}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [FailBannerModel] = //: return Array<FailBannerModel>()
        .init()
    //: }()
    ...
    private lazy var bannerView: JXBanner = {
        ...
        banner.placeholderImgView.image = UIImage.scaleValue()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(main_giftFormat.prefix(6)) + "hats" + String(notiFormatName.suffix(5)) + "er_su" + mEqualMsg.replacingOccurrences(of: "aspect", with: "ct"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.sendCollection(name: (String(dreamMakeValue) + String(k_modelUrl) + String(mLoadMsg.suffix(8)))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(aboveContentData), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension InsertBannerDelegate {
    /// banner
    //: func reqBanner() {
    func elite() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        ServerRequestManager().pattyPan(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(dream_quoteMsg.replacingOccurrences(of: "count", with: "e") + "rList")] ?? []).arrayValue
...
extension InsertBannerDelegate {
    ...
    @objc private func aboveContentData() {
        ...
        self.delegate?.gildTheLily(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func backgroundImage(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension InsertBannerDelegate: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (noti_equalId.replacingOccurrences(of: "value", with: "X") + String(m_interactionNamePath)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.sumercalate(urlStr: url, placeImg: UIImage.system())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(kColorModelAsName.prefix(6)) + String(mainInputData)))!
