 ...
 *
 *  EnableDelegate 协议和 MainViewController 类。
 *  MainViewController 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 MainViewController 的具体声明。
 *
 ******************************************************************************/
...
//: @class ReasonViewCell;
@class ReasonViewCell;
//: @class ButtonDataThen;
@class ButtonDataThen;
//: @class V2TIMMessage;
@class V2TIMMessage;

//: typedef NS_ENUM(NSInteger, DataIMChatType) {
typedef NS_ENUM(NSInteger, DataIMChatType) {
    //: DataIMChatType_unDefined = 0,
    DataIMChatType_unDefined = 0,
    //: DataIMChatType_private = 1,
    DataIMChatType_private = 1,
    //: DataIMChatType_group = 2,
    DataIMChatType_group = 2,
    //: DataIMChatType_system = 3,
    DataIMChatType_system = 3,
    //: DataIMChatType_service = 4, 
    DataIMChatType_service = 4, // 官方客服
    //: DataIMChatType_meeting = 5, 
    DataIMChatType_meeting = 5, // 公共聊天室
    //: DataIMChatType_moreMsg = 6, 
    DataIMChatType_moreMsg = 6, // 更多消息（超过5天未回复）
//: };
};
...
@protocol EnableDelegate <NSObject>
...
- (void)controllers:(MainViewController *)controller;
...
- (void)cameraing:(MainViewController *)controller;
...
- (BOOL)to:(MainViewController *)controller line:(UIView *)view;
 ...
 */
//: - (ButtonDataThen *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (ButtonDataThen *)betweenCommunication:(MainViewController *)controller to:(V2TIMMessage *)data;
 ...
 */
//: - (ReasonViewCell *)messageController:(TUIMessageController *)controller onShowMessageData:(ButtonDataThen *)data;
- (ReasonViewCell *)graphicIndex:(MainViewController *)controller data:(ButtonDataThen *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(ReasonViewCell *)cell;
- (void)load:(MainViewController *)controller can:(ReasonViewCell *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(ReasonViewCell *)cell;
- (void)headContent:(MainViewController *)controller app:(ReasonViewCell *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(ButtonDataThen *)data;
- (void)communicationTitle:(MainViewController *)controller sessionFor:(NSInteger)menuType user:(ButtonDataThen *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(ReasonViewCell *)cell;
- (void)beauty:(MainViewController *)controller peeler:(ReasonViewCell *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(ButtonDataThen *)cellData;
- (void)view:(MainViewController *)controller alongMessage:(ButtonDataThen *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(ButtonDataThen *)cellData;
- (void)pause:(MainViewController *)controller deviceData:(ButtonDataThen *)cellData;
...
/**
 *  执行 EnableDelegate 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<EnableDelegate> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<ButtonDataThen *> *)uiMsgs;
...
/**
 * 批量将V2TIMessage转换成ButtonDataThen
 */
...
///当前对话类型
//: @property (nonatomic, assign) DataIMChatType currentChatType;
@property (nonatomic, assign) DataIMChatType currentChatType;
