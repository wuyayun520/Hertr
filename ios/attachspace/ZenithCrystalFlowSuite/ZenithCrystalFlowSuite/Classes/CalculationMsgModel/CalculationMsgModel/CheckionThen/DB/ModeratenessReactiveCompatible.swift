...
/*: "serialQueue" :*/
fileprivate let m_byId:[Character] = ["s","e","r","i","a","l","Q","u","e","u","e"]

/*: "fromUids" :*/
fileprivate let m_managerText:[UInt8] = [0xae,0xba,0xa7,0xa5,0x9d,0xa1,0xac,0xbb]

private func equalLength(add num: UInt8) -> UInt8 {
    return num ^ 200
}
...
// __DEBUG__
// __CLOSE_PRINT__
//  ModeratenessReactiveCompatible.swift
//  AbroadTalking
//
...
let user_marginTitle = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class ModeratenessReactiveCompatible: NSObject {
    ...
    private static var _instance: ModeratenessReactiveCompatible?
    //: class func shared() -> DBUserInfoManager {
    class func fromRoot() -> ModeratenessReactiveCompatible {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = ModeratenessReactiveCompatible()
            // 添加退出账号通知
    ...
    @objc private func logout() {
        ...
        ModeratenessReactiveCompatible.development()
    }
}
...
extension ModeratenessReactiveCompatible {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([VisualCommunicationReactiveCompatible]?, Error?) -> Void) {
    class func addEnable(with userIdsArr: [String], completion: @escaping ([VisualCommunicationReactiveCompatible]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if ModeratenessReactiveCompatible.fromRoot().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(ModeratenessReactiveCompatible.fromRoot().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            ModeratenessReactiveCompatible.fromRoot().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            begin(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func sizeModel(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(m_byId)))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    SourceRequestTool.topOff(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func sourceLine(message: V2TIMMessage) -> CommentMsgTable {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = PrimrosePathThen.pathTo(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = CommentMsgTable.customer(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = PrimrosePathThen.stochasticProcess(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = CommentMsgTable.launchNight(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension ModeratenessReactiveCompatible {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> VisualCommunicationReactiveCompatible? {
    class func drippingPan(targetId: String) -> VisualCommunicationReactiveCompatible? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return CreateInfoTable.observerPlay(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: VisualCommunicationReactiveCompatible?) {
    class func nameClick(with userInfo: VisualCommunicationReactiveCompatible?) {
        ...
        let model = ModeratenessReactiveCompatible.drippingPan(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        CreateInfoTable.personDataInfo(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: userMaxEndPath, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func messagePath(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ModeratenessReactiveCompatible.drippingPan(targetId: targetId) else { return }
    ...
    class func insert(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ModeratenessReactiveCompatible.drippingPan(targetId: targetId) else { return }
    ...
    class func toFinish(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        CreateInfoTable.jam(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        ModeratenessReactiveCompatible.fromRoot().reqedUidsSet.remove(targetUid)
    }
}
...
extension ModeratenessReactiveCompatible {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([VisualCommunicationReactiveCompatible]?, Error?) -> Void) {
    private class func begin(with userIdsArr: [String], completion: @escaping ([VisualCommunicationReactiveCompatible]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = CreateInfoTable.listWith(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if ModeratenessReactiveCompatible.fromRoot().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            ModeratenessReactiveCompatible.sumerval(with: noDBUidArr)
        }
    }
    ...
    private class func sumerval(with userIdsArr: [String]) {
        ...
        ModeratenessReactiveCompatible.fromRoot().reqedUidsSet = ModeratenessReactiveCompatible.fromRoot().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TalkingChatRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        SourceRequestTool.overcharge(param: [String(bytes: m_managerText.map{equalLength(add: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                ModeratenessReactiveCompatible.fromRoot().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            ModeratenessReactiveCompatible.back(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: VisualCommunicationReactiveCompatible] = [:]
            var retInfoDict: [String: VisualCommunicationReactiveCompatible] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: userMaxEndPath, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [VisualCommunicationReactiveCompatible]) {
    private class func back(userInfoArr: [VisualCommunicationReactiveCompatible]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            CreateInfoTable.personDataInfo(aWrap)
        }
    }
}
