...
/*: "id" :*/
fileprivate let dream_selectedFormat:String = "ID"

/*: "num" :*/
fileprivate let user_popKey:[Character] = ["n","u","m"]
...
@objc public enum SightSignedNumeric: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct LabelMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// SightSignedNumeric
    ...
    mutating func bulge(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct BankManagerModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(user_popKey))
    }
}
