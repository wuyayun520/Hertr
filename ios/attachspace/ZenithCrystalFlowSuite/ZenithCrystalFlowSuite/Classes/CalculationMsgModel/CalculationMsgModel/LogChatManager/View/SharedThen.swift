...
fileprivate let dream_withMsg:[UInt8] = [0x88,0x8f,0x88,0x95,0xc9,0x82,0x8e,0x85,0x84,0x93,0xdb,0xc8,0xc1,0x89,0x80,0x92,0xc1,0x8f,0x8e,0x95,0xc1,0x83,0x84,0x84,0x8f,0xc1,0x88,0x8c,0x91,0x8d,0x84,0x8c,0x84,0x8f,0x95,0x84,0x85]

/*: "icon_mini_voice_end" :*/
fileprivate let dreamColorName:String = "icon_milab length log cancel"
fileprivate let notiCircleMessage:String = "ni_voiin kit type hidden"

/*: "#F95151" :*/
fileprivate let k_equalData:[Character] = ["#","F"]
fileprivate let mainShareTitle:String = "95gift5gift"

/*: "End of call" :*/
fileprivate let showMakeId:String = "End of effect touch press"
fileprivate let app_cellUpStr:String = "viewll"

/*: "img_home_shadow_icon" :*/
fileprivate let dreamDirectionId:String = "iequal"
fileprivate let noti_kitDialogTitle:String = "g_homedata frame make to text"
fileprivate let dreamEditId:String = "case tool view background viewow_icon"
...
class SharedThen: SourceMiniView {
	var keepSum: Int = 30
	var directorName: String = "app"
	var detailArray: [AnyHashable] = []
	var modeListDictionary: [AnyHashable: String] = [:]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_withMsg.map{$0^225}, encoding: .utf8)!)
    }
    ...
    override func modelEnd() {
        ...
        super.modelEnd()
            ...
            if (!shadowImg.transform.isIdentity) && (shadowImg.semanticContentAttribute == .playback) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let effectLet = DataLikeView(frame: shadowImg.frame.insetBy(dx: CGFloat(532.08), dy: CGFloat(537.89)))
            effectLet.blockOff = self.pointAnimView.isPlayAnimation
            
            effectLet.modelQuantity = { [self] messageQuantity in
            self.keepSum = messageQuantity
            
            return self.keepSum
            }
            effectLet.borderName = { [self] hypothesisContent in
            self.directorName = hypothesisContent
            
            return self.directorName
            }
            effectLet.dataArray = { [self] methodArray in
            self.detailArray = methodArray
            
            guard var value = self.detailArray as? [String] else {
                return nil
            }
            return value
            }
            effectLet.stationDictionary = { [self] upDictionary in
            self.modeListDictionary = upDictionary
            
            guard var value = self.modeListDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                shadowImg.addSubview(effectLet)
            }
    ...
    private lazy var statusIcon: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (String(dreamColorName.prefix(7)) + String(notiCircleMessage.prefix(6)) + "ce_end"))
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(k_equalData) + mainShareTitle.replacingOccurrences(of: "gift", with: "1")))
        //: lab.font = .pingfangFont(type: .Medium, fontSize: 10)
        lab.font = .bindFor(type: .Medium, fontSize: 10)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.text = "End of call".localized
        lab.text = (String(showMakeId.prefix(7)) + app_cellUpStr.replacingOccurrences(of: "view", with: "ca")).localized
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (dreamDirectionId.replacingOccurrences(of: "equal", with: "m") + String(noti_kitDialogTitle.prefix(6)) + "_shad" + String(dreamEditId.suffix(7))))
...
extension SharedThen {
    ...
    static func command() -> SharedThen {
        //: let view = TalkingVideoMiniView(frame: .zero)
        let view = SharedThen(frame: .zero)
