...
/*: "call_bgm" :*/
fileprivate let showBetweenId:String = "caequalequal"

/*: "call_response_bgm" :*/
fileprivate let dreamDataBottomContent:String = "cshapell"
fileprivate let dreamIconData:String = "nse_bgmbottom conversation normal"
...
enum CommonHashableRepresentation: Int {
    //: case Call
    case Call /// 音视频拨打音乐
    //: case Response
...
class ReasonWindowPlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = CommonHashableRepresentation.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func componentTo(type: CommonHashableRepresentation) -> ReasonWindowPlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = ReasonWindowPlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.ambagesPath()
...
extension ReasonWindowPlayer {
    ...
    private func upTheme(notify: Notification) {
    ...
    private func exitImage(notify: Notification) {
    ...
    private func ambagesPath() {
        //: var fileName = "call_bgm"
        var fileName = (showBetweenId.replacingOccurrences(of: "equal", with: "l") + "_bgm")
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (dreamDataBottomContent.replacingOccurrences(of: "shape", with: "a") + "_respo" + String(dreamIconData.prefix(7)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ReverberationInsideHoeHandleReactiveCompatible.default.smallBy(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension ReasonWindowPlayer: TXVodPlayListener {
