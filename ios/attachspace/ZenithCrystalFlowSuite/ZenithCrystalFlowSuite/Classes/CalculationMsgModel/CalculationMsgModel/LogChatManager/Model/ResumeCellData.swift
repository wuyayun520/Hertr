...
/*: ：" :*/
fileprivate let kViewUrl:String = "："

/*: "#BAFBFF" :*/
fileprivate let userManagerTitleIdent:String = "gift live#BAFBFF"
...
class ResumeCellData: NSObject {
    ...
    func afterUser(model: KibbitzModelType) -> KibbitzModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func changeComment() {
        ...
        let name = fillName(text: firstStr, font: UIFont.upperClassMove(fontSize: 14), color: UIColor(hex: (String(userManagerTitleIdent.suffix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = fillName(text: self.model.content, font: UIFont.upperClassMove(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        titleEqual(attribText: self.model.msgAttribText)
    }
    ...
    func fromClick() {
        ...
        let content = fillName(text: self.model.transContent, font: UIFont.upperClassMove(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func errorStyle() -> NSMutableParagraphStyle {
    ...
    func fillName(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = errorStyle()
        //: return attribute
        return attribute
    }
    ...
    func titleEqual(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = mainBarName - 36
