...
fileprivate let app_makePath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#FA9D33" :*/
fileprivate let dreamGoldText:String = "value leading#FA9D3"
fileprivate let appNameKey:[Character] = ["3"]

/*: "#343338" :*/
fileprivate let mSizeSecondIdent:String = "#343338"
...
class ShareViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_makePath.reversed(), encoding: .utf8)!)
    }
...
extension ShareViewCell {
    /// 刷新cell
    /// - Parameters:
    ///   - position: 麦位
    ///   - model: 麦位数据
    //: func refreshIcon(position: Int, model: TalkingMikeListItemModel) {
    func forefront(position: Int, model: PortMeasurable) {
        //: guard model.uid > 0, String(model.uid) != PathAppManager.share.loginUid else {
        guard model.uid > 0, String(model.uid) != PathAppManager.share.loginUid else {
        ...
        let color = model.needGift ? UIColor(hex: (String(dreamGoldText.suffix(6)) + String(appNameKey))) : UIColor(hex: (mSizeSecondIdent.capitalized))
