...
fileprivate let mTopMsg:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

private func cellTo(show num: UInt8) -> UInt8 {
    return num ^ 118
}

/*: "bth_live_follow_pre" :*/
fileprivate let kAspectValue:[Character] = ["b","t","h","_","l","i","v","e","_","f","o","l","l","o","w","_","p","r"]
fileprivate let notiEraseId:String = "output"

/*: "btn_live_closed_nor" :*/
fileprivate let mainFilterId:String = "white clear view text onbtn_l"
fileprivate let mTextContent:String = "ive_cloapp self size equal"
fileprivate let userMakeViewName:[Character] = ["s","e","d","_","n","o","r"]

/*: "removeAttentionUid" :*/
fileprivate let dream_currentValue:[Character] = ["r","e","m","o","v","e","A","t","t","e","n","t","i","o","n","U","i"]
fileprivate let mainIndexModelMessage:[Character] = ["d"]

/*: "attentionUid" :*/
fileprivate let notiAttentionFormat:String = "succeed"
fileprivate let notiStateDataUrl:[Character] = ["t","e","n","t","i","o","n","U","i","d"]

/*: "source" :*/
fileprivate let app_requestData:String = "price"
fileprivate let m_activityName:String = "lineurce"

/*: "num" :*/
fileprivate let show_collectTitle:String = "narraym"

/*: "99+" :*/
fileprivate let mFromMessage:[UInt8] = [0xca,0xca,0xd8]

private func appRange(interval num: UInt8) -> UInt8 {
    return num ^ 243
}

/*: "UID: :*/
fileprivate let appBarData:[Character] = ["U","I","D",":"]
...
class ColorTopView: UIView {
    ...
    private var partyModel = GrantTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mTopMsg.map{cellTo(show: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(addEqual), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(kAspectValue) + notiEraseId.replacingOccurrences(of: "output", with: "e"))), for: .normal)
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(mainFilterId.suffix(5)) + String(mTextContent.prefix(7)) + String(userMakeViewName))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(inscription), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(secondServer), for: .touchUpInside)
    ...
    private lazy var onlineListView: LogViewDelegate = {
        //: let v = TalkingVoiceRoomOnlineListView()
        let v = LogViewDelegate()
...
extension ColorTopView {
    ...
    @objc func translate(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(dream_currentValue) + String(mainIndexModelMessage))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(notiAttentionFormat.replacingOccurrences(of: "succeed", with: "at") + String(notiStateDataUrl))] = uid
            //: dict["source"] = "2"
            dict[(app_requestData.replacingOccurrences(of: "price", with: "s") + m_activityName.replacingOccurrences(of: "line", with: "o"))] = "2"
        }
...
extension ColorTopView {
    ...
    @objc private func dark() {
        ...
        CharterReqTool.startStop(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(show_collectTitle.replacingOccurrences(of: "array", with: "u"))].intValue
    ...
    private func colorSub() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(dark), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension ColorTopView {
    ...
    func kill() {
        ...
        partyModel = VoxNameReactiveCompatible.listOf().partyModel
        //: nickNameLab.text = partyModel.streamerInfo.nickname
        nickNameLab.text = partyModel.streamerInfo.nickname
        //: uidLab.text = "UID:\(partyModel.streamerInfo.uid)"
        uidLab.text = (String(appBarData)) + "\(partyModel.streamerInfo.uid)"
        ...
        icon.sumercalate(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.indicatorSex(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == PathAppManager.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == PathAppManager.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            anObject(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            anObject(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        colorSub()
    ...
    @objc private func userClick() {
        ...
        translate(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.anObject(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }

    /// 最小化按钮点击事件
    //: @objc private func miniButtonClick() {
    @objc private func inscription() {
        //: superview?.endEditing(true)
        superview?.endEditing(true)
        //: let view = TalkingVoiceRoomCloseView()
        let view = ForefrontOpenView()
        //: view.show()
        view.makeSize()
    }
    ...
    func anObject(isAttention: Bool) {
    ...
    @objc private func secondServer() {
        //: onlineListView.showView()
        onlineListView.currentView()
    }
}
...
extension ColorTopView {
    ...
    private func backConstraint() {
        ...
        frame = CGRect(x: 0, y: 0, width: app_screenFailMarginFormat, height: app_topMsg + actualWidth(w: 43))
