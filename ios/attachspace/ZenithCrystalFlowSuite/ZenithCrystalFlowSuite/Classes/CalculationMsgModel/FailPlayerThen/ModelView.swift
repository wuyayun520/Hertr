...
fileprivate let user_viewWhiteIdent:[Character] = ["设","置","普","通","状","态","的","文","字","颜","色","时"," ","请","使","用","R","G","B","\u{7a7a}","间","的","\u{989c}","\u{8272}"]
fileprivate let noti_bagPath:String = "at"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let main_viewKey:[Character] = ["设","置","选","中","状","态","\u{7684}","文","字","颜","色","时"," ","\u{8bf7}","使","用","R","G","B","空","间","的","\u{989c}","色","\u{503c}"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let mainMakeData:[UInt8] = [0xd7,0xd0,0xd7,0xca,0x96,0xdd,0xd1,0xda,0xdb,0xcc,0x84,0x97,0x9e,0xd6,0xdf,0xcd,0x9e,0xd0,0xd1,0xca,0x9e,0xdc,0xdb,0xdb,0xd0,0x9e,0xd7,0xd3,0xce,0xd2,0xdb,0xd3,0xdb,0xd0,0xca,0xdb,0xda]

private func forPathReport(cookie num: UInt8) -> UInt8 {
    return num ^ 190
}

/*: "设置的下标不合法!!" :*/
fileprivate let user_messageMoveUrl:[Character] = ["设","置","的","下","标","\u{4e0d}","\u{5408}","法","!","!"]

/*: "Bags" :*/
fileprivate let userVideoKey:[Character] = ["B","a","g","s"]
...
open class ModelView: UIView {
	var voiceOff: Bool = true
	var equalCount: Double = 54.1
	var appearArray: [AnyHashable] = []
	var originDictionary: [AnyHashable: String] = [:]
	var disappearEnable: Bool = false
	var hideTotalQuantity: Double = -33.4
	var cellArray: [AnyHashable] = []
	var sumRelationDictionary: [AnyHashable: String] = [:]

    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: SubsectionGiftDrapeSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.discredit(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(user_viewWhiteIdent) + noti_bagPath.replacingOccurrences(of: "at", with: "值")))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.discredit(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(main_viewKey)))
        }
    ...
    fileprivate func discredit(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
	...
	if let scrollLine = scrollLine {

            if (scrollLine.backgroundColor != nil && scrollLine.backgroundColor!.cgColor == UIColor(cgColor: UIColor.gray.cgColor).cgColor) && (scrollLine.layer.zPosition == 28.66) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let dismissRow = CurRequireView()

            dismissRow.sizeListOn = { [self] streamOn in
            self.disappearEnable = streamOn
            
            var dismissRow = self.segmentStyle.showLine
                dismissRow = false
                dismissRow = !dismissRow
            if dismissRow != self.disappearEnable {
                self.disappearEnable = dismissRow
            }
            
            return self.disappearEnable
            }
            dismissRow.libraryLabMagnitude = { [self] pushMagnitude in
            self.hideTotalQuantity = pushMagnitude
            
            return self.hideTotalQuantity
            }
            dismissRow.increaseArray = { [self] genreSumArray in
            self.cellArray = genreSumArray
            ...
            self.sumRelationDictionary = rowDictionary
            
            guard var value = self.sumRelationDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                scrollLine.addSubview(dismissRow)
            }
    ...
    public init(frame: CGRect, segmentStyle: SubsectionGiftDrapeSegmentStyle, titles: [String]) {
    ...
    @objc func recount(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? LiveReactiveCompatible else { return }
        ...
        information(true)
            ...
            if (self.backgroundColor != nil && self.backgroundColor!.cgColor == UIColor(cgColor: UIColor.gray.cgColor).cgColor) && (self.layer.zPosition == 28.66) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let dismissRow = CurRequireView()

            dismissRow.sizeListOn = { [self] streamOn in
            self.voiceOff = streamOn
            
            var dismissRow = segmentStyle.showCover
                dismissRow = false
                dismissRow = !dismissRow
            if dismissRow != self.voiceOff {
                self.voiceOff = dismissRow
            }
            
            return self.voiceOff
            }
            dismissRow.libraryLabMagnitude = { [self] pushMagnitude in
            self.equalCount = pushMagnitude
            
            return self.equalCount
            }
            dismissRow.increaseArray = { [self] genreSumArray in
            self.appearArray = genreSumArray
            ...
            dismissRow.atDictionary = { [self] rowDictionary in
            self.originDictionary = rowDictionary
            
            guard var value = self.originDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                self.addSubview(dismissRow)
            }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainMakeData.map{forPathReport(cookie: $0)}, encoding: .utf8)!)
    }
}
...
public extension ModelView {
    ...
    func rowClickAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(user_messageMoveUrl)))
    ...
    func addNames(_ titles: [String]) {
        ...
        rowClickAnimated(0, animated: true)
    }
}
...
extension ModelView {
    ...
    fileprivate func become() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = LiveReactiveCompatible(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.recount(_:)))
            ...
            if title == (String(userVideoKey)).localized {
    ...
    fileprivate func bank() {
        ...
        if let firstLabel = labelsArray[0] as? LiveReactiveCompatible {
    ...
    public func eventAdjust(_ currentIndex: Int) {
...
open class LiveReactiveCompatible: UILabel {
    /// 用来记录当前label的缩放比例
