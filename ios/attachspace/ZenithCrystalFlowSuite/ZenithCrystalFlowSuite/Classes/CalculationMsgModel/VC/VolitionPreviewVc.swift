...
fileprivate let app_bottomValue:[UInt8] = [0x2e,0x29,0x2e,0x33,0x6f,0x24,0x28,0x23,0x22,0x35,0x7d,0x6e,0x67,0x2f,0x26,0x34,0x67,0x29,0x28,0x33,0x67,0x25,0x22,0x22,0x29,0x67,0x2e,0x2a,0x37,0x2b,0x22,0x2a,0x22,0x29,0x33,0x22,0x23]

/*: "icon_photo_giftbg" :*/
fileprivate let mSizeValue:String = "imodelon"
fileprivate let userHiddenIdent:String = "to_giftbgvideo current view self"

/*: "Send" :*/
fileprivate let noti_viewLeadingPath:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let kDetailStr:String = "nav_bmini at"
fileprivate let dreamRowEqualPath:String = "ack_bvoice view touch icon for"

/*: "toUid" :*/
fileprivate let m_actionId:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let user_byValue:String = "content selfgiftId"

/*: "giftNum" :*/
fileprivate let appBottomTitle:[Character] = ["g","i","f","t","N","u"]
fileprivate let userTagName:String = "M"

/*: "unlockMsgId" :*/
fileprivate let appErrorData:String = "UNLOCK"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let showTopId:[UInt8] = [0xf5,0xde,0xde,0xdf,0x9b,0xda,0x9b,0x9e,0xfb,0x9b,0x93,0x9e,0xfb,0x9b,0xfc,0xd4,0xd7,0xdf,0x9b,0xd8,0xd4,0xd2,0xd5,0xc8,0x92,0x9b,0xcf,0xd4,0x9b,0xce,0xd5,0xd7,0xd4,0xd8,0xd0]

private func sizeFemaleImage(pic num: UInt8) -> UInt8 {
    return num ^ 187
}

/*: "icon_photo_lock" :*/
fileprivate let user_labData:String = "equal safe changeicon_"
fileprivate let k_videoIdent:String = "pic log_loc"
fileprivate let mElementContent:[Character] = ["k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let dreamLengthBlockReturnId:[UInt8] = [0x91,0xba,0xb7,0xb1,0xb9,0x6e,0xbd,0xbc,0x6e,0xc2,0xb6,0xb3,0x6e,0xc1,0xb1,0xc0,0xb3,0xb3,0xbc,0x6e,0xc2,0xbd,0x6e,0xc4,0xb7,0xb3,0xc5,0x6e,0x7f,0x83,0xc1]

fileprivate func atPath(until num: UInt8) -> UInt8 {
...
/*: "icon_photo_burn" :*/
fileprivate let showGiftPath:String = "icon_phformat transform model"
fileprivate let m_labelKey:[Character] = ["o","t","o","_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let noti_dataValue:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r","n","e"]
fileprivate let showPathData:[Character] = ["d"]
...
typealias IntimatePhotoBlock = (_ model: StochasticProcessModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class VolitionPreviewVc: ListRecognizerDelegate {
	var totalMasterSum: Int = 78
	var fenceMagnitude: Double = 79.4
	var heartinessTotal: Int = 85
	var iconQuantity: Double = -76.5
	var modelQuantity: Int = 3
	var rowTotal: Double = -21.0
    ...
    //: private var imageTotalData = ChatThen(direction: .MsgDirectionOutgoing)
    private var imageTotalData = ChatThen(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = StochasticProcessModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: ChatThen, toUid: String) {
    init(imageCellData: ChatThen, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(PathAppManager.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(PathAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_bottomValue.map{$0^71}, encoding: .utf8)!)
    }
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        messUp(isOpen: true)
            ...
            if (desLab.layer.position.y == 69.74) && (desLab.mask != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let lessFormat = UpwardsView()
            lessFormat.messageUntilDoing = isMySelf
            
            lessFormat.momentTitleLabCount = { [self] viewInterval in
            self.totalMasterSum = viewInterval
            
            var lessFormat = imageTotalData.messageType
                lessFormat -= 1
                if lessFormat != 72 {
                    lessFormat = lessFormat + 1
                }
            if lessFormat < self.totalMasterSum {
                self.totalMasterSum = lessFormat
            }
            
            return self.totalMasterSum
            }
            lessFormat.itemNumber = { [self] modelNumber in
            self.fenceMagnitude = modelNumber
            
            self.fenceMagnitude /= 5
            return self.fenceMagnitude
            }
                desLab.addSubview(lessFormat)
            }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        messUp(isOpen: false)
            ...
            if (giftIcon.layer.position.y == 69.74) && (giftIcon.mask != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let lessFormat = UpwardsView()
            lessFormat.messageUntilDoing = self.hideNavi
            
            lessFormat.momentTitleLabCount = { [self] viewInterval in
            self.heartinessTotal = viewInterval
            ...
            if lessFormat < self.heartinessTotal {
                self.heartinessTotal = lessFormat
            }
            
            return self.heartinessTotal
            }
            lessFormat.itemNumber = { [self] modelNumber in
            self.iconQuantity = modelNumber
            
            self.iconQuantity /= 5
            return self.iconQuantity
            }
                giftIcon.addSubview(lessFormat)
            }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (mSizeValue.replacingOccurrences(of: "model", with: "c") + "_pho" + String(userHiddenIdent.prefix(9))))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.glycerolizeNeed(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(noti_viewLeadingPath)).localized, for: .normal)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.sendCollection(name: (String(kDetailStr.prefix(5)) + String(dreamRowEqualPath.prefix(5)) + "lack_nor")).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(adpressed), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension VolitionPreviewVc {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func titlePush(giftId: String, giftNum: String) {
        ...
        params[(String(m_actionId))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(user_byValue.suffix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(appBottomTitle) + userTagName.lowercased())] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(appErrorData.lowercased() + "MsgId")] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension VolitionPreviewVc {
    ...
    @objc private func fixing() {
        ...
        titlePush(giftId: photoModel.giftId, giftNum: "1")
    }
    ...
    @objc private func choose() {
        ...
        self.quantityersection(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.selectCountoVia(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension VolitionPreviewVc {
    ...
    private func infoTo() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: showTopId.map{sizeFemaleImage(pic: $0)}, encoding: .utf8)!.equalArguments(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.sendCollection(name: (String(user_labData.suffix(5)) + "photo" + String(k_videoIdent.suffix(4)) + String(mElementContent)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: dreamLengthBlockReturnId.map{atPath(until: $0)}, encoding: .utf8)!.localized
                // 添加手势
                ...
                self.intimateStatusImgV.image = UIImage.sendCollection(name: (String(showGiftPath.prefix(7)) + String(m_labelKey)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(noti_dataValue) + String(showPathData)).localized
            }
        ...
        self.quantityersection(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func quantityersection(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
