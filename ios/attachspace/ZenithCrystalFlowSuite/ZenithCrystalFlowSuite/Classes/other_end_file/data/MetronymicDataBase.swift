...
class MetronymicDataBase: NSObject {
    static let sharedInstance = MetronymicDataBase()
    private lazy var rootPath: URL = {
        ...
        let path = cachesDirectory.appendingPathComponent("/selected.db")
    ...
    lazy var data: [String: [String: MetronymicDataModelBase]] = {
        let decoder = JSONDecoder()
        if let json = FileUtil.readFile(filePath: rootPath), let value = try? decoder.decode([String: [String: String]].self, from: json) {
            var result: [String: [String: MetronymicDataModelBase]] = [:]
            for (tableName, table) in value {
                result[tableName] = [:]
                for (primaryKey, model) in table {
                    if let cls = MetronymicTool.Quantityclass(from: primaryKey) as? MetronymicDataModelBase.Type,
...
protocol MetronymicDataProtocol {
...
class MetronymicDataModelBase: Codable, MetronymicDataProtocol {
