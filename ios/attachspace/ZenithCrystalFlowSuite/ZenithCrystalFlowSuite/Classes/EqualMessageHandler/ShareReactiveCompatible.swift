...
/*: "&sr=6" :*/
fileprivate let userMainName:[Character] = ["&","s","r","=","6"]
...
enum SelectionTableConvertible: String, CaseIterable {
...
enum LogVideoNameProtocol: String {
    ...
    case luckyBlindBox = "/dist/instruction/luckyBlindBox.html" // 盲盒说明页
    //: case Intimate         = "/dist/honey/index.html"
    case Intimate = "/dist/honey/index.html"
    // 1.7.6 亲密度
}

//: class TalkingWebManager: NSObject {
class ShareReactiveCompatible: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func isText(urlType: LogVideoNameProtocol) -> String {
        //: var url = PathAppManager.share.appConfigMode.urlH5Domain
        var url = PathAppManager.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(PathAppManager.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : PathAppManager.share.appUserConfigMode.fullRechargePage)
            url.append(PathAppManager.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : PathAppManager.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(PathAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : PathAppManager.share.appUserConfigMode.halfRechargePage)
            url.append(PathAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : PathAppManager.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = PathAppManager.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = PathAppManager.share.appUserConfigMode.halfRechargePage.appending((String(userMainName)))
            //: url.append(PathAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(PathAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
