...
fileprivate let m_throughModelData:[UInt8] = [0xd,0xa,0xd,0x10,0x4c,0x7,0xb,0x0,0x1,0x16,0x5e,0x4d,0x44,0xc,0x5,0x17,0x44,0xa,0xb,0x10,0x44,0x6,0x1,0x1,0xa,0x44,0xd,0x9,0x14,0x8,0x1,0x9,0x1,0xa,0x10,0x1,0x0]

private func nameGroup(progress num: UInt8) -> UInt8 {
    return num ^ 100
}

/*: "Enter phone number" :*/
fileprivate let show_indexPath:[Character] = ["E","n","t","e","r"," ","p","h","o","n","e"," ","n"]
fileprivate let show_pageData:[Character] = ["u","m","b","e","r"]

/*: "Enter Email Address" :*/
fileprivate let app_attentionContent:String = "each error mode image colorEnter"
fileprivate let dreamRawText:[Character] = [" ","E","m","a","i","l"," ","A","d"]
fileprivate let main_managerTableIdent:String = "dremode"

/*: "F4F4F4" :*/
fileprivate let noti_arrayFormat:[Character] = ["F","4","F","4","F","4"]

/*: "America(+1)" :*/
fileprivate let noti_showTitle:[Character] = ["A"]
fileprivate let m_infoIdent:String = "instanceerica"

/*: "area_code_down_icon" :*/
fileprivate let user_blockValue:String = "area_cofalse app"
fileprivate let appCloseIdent:[Character] = ["d","e","_","d","o"]
fileprivate let user_contentFormat:String = "wn_iconuser model"

/*: "BBBBBB" :*/
fileprivate let user_labelId:String = "finishfinishfinish"

/*: "btn_field_delete_icon" :*/
fileprivate let userDataMessage:String = "btn_fsize by content"
fileprivate let mainImageIdent:String = "count group pro button_delet"
fileprivate let dreamPlainMessage:String = "e_iconcontent bar"

/*: "Password" :*/
fileprivate let kStartName:String = "Papoint player for"
fileprivate let dream_modelFormat:[Character] = ["s","s","w","o","r","d"]

/*: "CCCCCC" :*/
fileprivate let notiContentId:String = "gogogogogogo"

/*: "or" :*/
fileprivate let kHiddenStr:[Character] = ["o","r"]

/*: "F5F5F5" :*/
fileprivate let noti_observerData:String = "Freceivereceive5"

/*: "Next" :*/
fileprivate let userExtraValue:[Character] = ["N","e","x","t"]

/*: "Login by password" :*/
fileprivate let noti_imageData:[Character] = ["L","o","g","i","n"," ","b","y"," "]
fileprivate let m_picTitle:String = "parequest"
fileprivate let notiMediumFormat:String = "swoeffect"

/*: "Login by OTP" :*/
fileprivate let notiAddKey:[Character] = ["L","o","g","i","n"," ","b","y"," ","O","T","P"]

/*: "#8C7AFF" :*/
fileprivate let app_topTitle:String = "by app app make case#8C7AFF"

/*: "Login" :*/
fileprivate let kMakeId:[Character] = ["L","o","g","i","n"]

/*: "Send verification" :*/
fileprivate let dreamNameTitle:String = "frame view giftSend"
fileprivate let main_suiteId:String = "originforiginc"
fileprivate let k_labValue:[Character] = ["n"]
...
class ConnectionThen: UIView, UITextFieldDelegate {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_throughModelData.map{nameGroup(progress: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLabel: UILabel = {
        ...
        label.text = self.loginType == .LoginPhone ? (String(show_indexPath) + String(show_pageData)).localized : (String(app_attentionContent.suffix(5)) + String(dreamRawText) + main_managerTableIdent.replacingOccurrences(of: "mode", with: "ss")).localized
        //: return label
        return label
        //: }()
    }()
    ...
    private lazy var areaCodeControl: UIControl = {
        ...
        control.backgroundColor = UIColor(hex: (String(noti_arrayFormat)))
    ...
    lazy var areaCodeLabel: UILabel = {
        ...
        label.text = (String(noti_showTitle) + m_infoIdent.replacingOccurrences(of: "instance", with: "m") + "(+1)")
        //: return label
        return label
        //: }()
    }()
    ...
    private lazy var areaDownImgView: UIImageView = {
        ...
        imageView.image = UIImage.sendCollection(name: (String(user_blockValue.prefix(7)) + String(appCloseIdent) + String(user_contentFormat.prefix(7))))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    private lazy var phoneInputView: UITextField = {
        ...
        textField.backgroundColor = UIColor(hex: (String(noti_arrayFormat)))
        ...
        textField.attributedPlaceholder = NSAttributedString(string: "", attributes: [NSAttributedString.Key.font: UIFont.bindFor(type: .Regular, fontSize: 17), NSAttributedString.Key.foregroundColor: UIColor(hex: (user_labelId.replacingOccurrences(of: "finish", with: "BB")))!])
    ...
    private lazy var phoneDeleteBtn: UIButton = {
        ...
        btn.setImage(UIImage.sendCollection(name: (String(userDataMessage.prefix(5)) + "ield" + String(mainImageIdent.suffix(6)) + String(dreamPlainMessage.prefix(6)))), for: .normal)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var pwdTitleLab: UILabel = {
        ...
        lab.text = (String(kStartName.prefix(2)) + String(dream_modelFormat)).localized
        //: lab.textColor = .appTitleColor()
        lab.textColor = .statusBy()
        //: lab.font = UIFont.pingfangFont(type: .Semibold, fontSize: 17)
        lab.font = UIFont.bindFor(type: .Semibold, fontSize: 17)
        //: lab.isHidden = true
        lab.isHidden = true
        //: return lab
        return lab
    ...
    private lazy var pwdInputView: UITextField = {
        ...
        textField.font = .bindFor(type: .Regular, fontSize: 17)
        //: textField.textColor = .appTitleColor()
        textField.textColor = .statusBy()
        //: textField.layer.cornerRadius = 25
        textField.layer.cornerRadius = 25
    ...
    lazy var otherLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (notiContentId.replacingOccurrences(of: "go", with: "C")))
    ...
    lazy var otherLeftLine: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (noti_observerData.replacingOccurrences(of: "receive", with: "5F")))
        //: return view
        return view
        //: }()
    }()
    ...
    lazy var commitButton: UIButton = {
        ...
        btn.setTitle((String(userExtraValue)).localized, for: .normal)
    ...
    private lazy var loginStyleBtn: UIButton = {
        ...
        btn.setTitleColor(UIColor.theBeauty(), for: .selected)
        //: btn.setTitle("Login by password".localized, for: .normal)
        btn.setTitle((String(noti_imageData) + m_picTitle.replacingOccurrences(of: "request", with: "s") + notiMediumFormat.replacingOccurrences(of: "effect", with: "rd")).localized, for: .normal)
        //: btn.setTitle("Login by OTP".localized, for: .selected)
        btn.setTitle((String(notiAddKey)).localized, for: .selected)
        //: btn.addTarget(self, action: #selector(loginStyleButtonClick(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(estheticalDrape(sender:)), for: .touchUpInside)
        //: btn.layer.cornerRadius = 25
        btn.layer.cornerRadius = 25
        //: btn.layer.borderWidth = 2
        btn.layer.borderWidth = 2
        //: btn.layer.borderColor = UIColor(hex: "#8C7AFF")?.withAlphaComponent(0.6).cgColor
        btn.layer.borderColor = UIColor(hex: (String(app_topTitle.suffix(7))))?.withAlphaComponent(0.6).cgColor
        //: return btn
        return btn
        //: }()
    }()
...
extension ConnectionThen {
    ...
    @objc private func estheticalDrape(sender: UIButton) {
        ...
        if sender.isSelected == true { // pwd
            ...
            commitButton.setTitle((String(kMakeId)).localized, for: .normal)
            //: } else {
        } else { // OTP
            //: commitButton.setTitle("Next".localized, for: .normal)
            commitButton.setTitle((String(userExtraValue)).localized, for: .normal)
        }
    ...
    func rangeWithTarget() {
        ...
        commitButton.setTitle((String(dreamNameTitle.suffix(4)) + " ver" + main_suiteId.replacingOccurrences(of: "origin", with: "i") + "atio" + String(k_labValue)).localized, for: .normal)
    }
}
