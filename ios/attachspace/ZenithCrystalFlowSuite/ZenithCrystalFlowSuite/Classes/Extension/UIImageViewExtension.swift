...
/*: "add_header_cookies" :*/
fileprivate let app_keyCellName:String = "aformat"
fileprivate let mainGlobalLeadingConversationUrl:[Character] = ["d","_","h","e","a","d","e","r","_","c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func glycerolizeNeed(urlStr: String,
    ...
    internal func sumercalate(urlStr: String,
                              //: placeImg: UIImage = UIImage.placeImgSquare(),
                              placeImg: UIImage = UIImage.system(),
    ...
    @objc func evidence(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: user_topData.disobey(), with: dreamRecordPath)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.viewGift())
    }
    ...
    @objc func stack(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: LiveAddrThen.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (app_keyCellName.replacingOccurrences(of: "format", with: "d") + String(mainGlobalLeadingConversationUrl)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.system(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
