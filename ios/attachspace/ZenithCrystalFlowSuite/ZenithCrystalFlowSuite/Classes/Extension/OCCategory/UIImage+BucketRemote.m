...
@implementation UIImage (BucketRemote)

//: - (NSData *)compressedImageData {
- (NSData *)pause {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.motionIcon;
...
- (UIImage *)afterIdentity {
    ...
    return [UIImage imageWithData:[self pause]];
}
...
- (UIImage *)filter:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self image:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self observer:size];
...
- (CGSize)image:(Boolean)isSession {
...
+ (NSData *)layer:(UIImage *)image pastIcon:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG localTranslate:scalesize attitude:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG pause:(fileSize*1024)];
    }
...
- (UIImage *)localTranslate:(CGSize)targetSize attitude:(BOOL)isScale{
...
- (NSData *)pause:(NSInteger)fileSize{
...
- (UIImage *)observer:(CGSize)newSize {
