...
fileprivate let k_labUrl:[UInt8] = [0x17,0x10,0x17,0xa,0x56,0x1d,0x11,0x1a,0x1b,0xc,0x44,0x57,0x5e,0x16,0x1f,0xd,0x5e,0x10,0x11,0xa,0x5e,0x1c,0x1b,0x1b,0x10,0x5e,0x17,0x13,0xe,0x12,0x1b,0x13,0x1b,0x10,0xa,0x1b,0x1a]

/*: "gift_bg_dm" :*/
fileprivate let mIntervalItemMsg:String = "gcurrentt"
fileprivate let mainRequestStr:String = "dshow"

/*: "#FFF4B5" :*/
fileprivate let showSendKey:[Character] = ["#","F"]
fileprivate let noti_imageKey:String = "toptop4B5"

/*: "won " :*/
fileprivate let mainGiftName:String = "won application return self let"

/*: " coins in " :*/
fileprivate let userHandleValue:String = "self self coins in"
fileprivate let show_makeTouchId:String = "temp"

/*: " x :*/
fileprivate let showEqualPath:String = "manager time index color main x"

/*: "just sent" :*/
fileprivate let app_iconUrl:[Character] = ["j","u","s","t"," ","s","e"]
fileprivate let app_labIdent:String = "nmain"

/*: "  :*/
fileprivate let appEntryMessage:String = "array"

/*: "#4120C1" :*/
fileprivate let k_numberName:[Character] = ["#","4","1","2","0","C"]
fileprivate let app_cellValue:[Character] = ["1"]
...
class GiftPortPanoramicView: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = CharterRateBroadcastModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_labUrl.map{$0^126}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.sendCollection(name: (mIntervalItemMsg.replacingOccurrences(of: "current", with: "if") + "_bg_" + mainRequestStr.replacingOccurrences(of: "show", with: "m")))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(showSendKey) + noti_imageKey.replacingOccurrences(of: "top", with: "F")))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(showSendKey) + noti_imageKey.replacingOccurrences(of: "top", with: "F")))
...
extension GiftPortPanoramicView {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func utiliserGift(_ model: CharterRateBroadcastModel) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.glycerolizeNeed(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(mainGiftName.prefix(4))).localized + "\(giftModel.totalCoin)" + (String(userHandleValue.suffix(9)) + show_makeTouchId.replacingOccurrences(of: "temp", with: " ")).localized + giftModel.gameName
            ...
            giftImgV.glycerolizeNeed(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(app_iconUrl) + app_labIdent.replacingOccurrences(of: "main", with: "t")).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = onSource(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(k_numberName) + String(app_cellValue)))!, range: nsRange)
            }
    ...
    private func onSource(giftmodel: CharterRateBroadcastModel) -> String {
...
extension GiftPortPanoramicView {
    ...
    private func giftApp() {
        ...
        self.frame = CGRect(x: 0, y: dream_systemTitle, width: app_screenFailMarginFormat, height: 60)
