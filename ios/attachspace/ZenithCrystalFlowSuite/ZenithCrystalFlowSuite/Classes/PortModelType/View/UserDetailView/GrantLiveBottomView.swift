...
fileprivate let userFirstSectionData:[UInt8] = [0x8e,0x89,0x8e,0x93,0xcf,0x84,0x88,0x83,0x82,0x95,0xdd,0xce,0xc7,0x8f,0x86,0x94,0xc7,0x89,0x88,0x93,0xc7,0x85,0x82,0x82,0x89,0xc7,0x8e,0x8a,0x97,0x8b,0x82,0x8a,0x82,0x89,0x93,0x82,0x83]

private func stochasticProcess(shared num: UInt8) -> UInt8 {
    return num ^ 231
}

/*: "bth_me_chat_nor" :*/
fileprivate let k_viewMsg:String = "bth_main view input your"
fileprivate let app_snapData:String = "t_norname make feature model scale"

/*: "#6ADEFF" :*/
fileprivate let userRangeName:[Character] = ["#"]
fileprivate let dream_giftSizeIdent:String = "6ADEFFreturn super if item conversation"

/*: "#5D6FFF" :*/
fileprivate let showKitPicMessage:[Character] = ["#","5","D","6","F","F"]
fileprivate let app_cookieMessage:[Character] = ["F"]

/*: "icon_lounge" :*/
fileprivate let showHiddenId:[Character] = ["i","c","o","n"]
fileprivate let mainPathValue:String = "_loungecolor head source"

/*: "icon_video_call" :*/
fileprivate let notiInfoFatalFormat:String = "icon_vidskin string area type return"
fileprivate let showMessageGiftResultValue:[Character] = ["e","o","_","c","a","l","l"]

/*: "iv_me_vip" :*/
fileprivate let k_appMessage:String = "data record model likeiv_me_vi"
fileprivate let user_clearRequestVoiceName:[Character] = ["p"]

/*: "Edit" :*/
fileprivate let appTopIdent:String = "Editview data from model"

/*: "#AB57F6" :*/
fileprivate let m_finishData:String = "row"
fileprivate let kGestureCenterFormat:String = "AB57Finput"

/*: "#FC57B7" :*/
fileprivate let main_liveName:[Character] = ["#","F","C","5","7","B","7"]

/*: "#FE9074" :*/
fileprivate let main_locationPath:String = "mask app face app input#FE9074"

/*: "Video Call" :*/
fileprivate let mainContentPath:[Character] = ["V","i","d","e","o"," ","C","a","l"]
fileprivate let showOkContent:[Character] = ["l"]
...
let userCameraKey = (15 + actualWidth(w: 36) + 15 + dreamOkTitle)

//: class TalkingUserDetailBottomView: UIView {
class GrantLiveBottomView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userFirstSectionData.map{stochasticProcess(shared: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var shadowLayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: app_screenFailMarginFormat, height: 15 + actualWidth(w: 38) + 15)
    ...
    private lazy var chatBtn: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (String(k_viewMsg.prefix(4)) + "me_cha" + String(app_snapData.prefix(5))))
    ...
    private lazy var videoBtn: UIButton = {
        ...
        let colors = [UIColor(hex: (String(userRangeName) + String(dream_giftSizeIdent.prefix(6))))!.cgColor, UIColor(hex: (String(showKitPicMessage) + String(app_cookieMessage)))!.cgColor]
        ...
        btn.setBackgroundImage(UIImage.garrison(colors: colors, size: CGSize(width: actualWidth(w: 250), height: actualWidth(w: 36))), for: .normal)
        //: btn.layer.cornerRadius = actualWidth(w: 18)
        btn.layer.cornerRadius = actualWidth(w: 18)
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(videoButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(untilLast), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (String(showHiddenId) + String(mainPathValue.prefix(7))))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var callIcon: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (String(notiInfoFatalFormat.prefix(8)) + String(showMessageGiftResultValue)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var maleVipPriceBtn: UIImageView = {
        ...
        img.image = UIImage.sendCollection(name: (String(k_appMessage.suffix(8)) + String(user_clearRequestVoiceName)))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(maleVipPriceButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(toClick))
        //: img.addGestureRecognizer(tap)
        img.addGestureRecognizer(tap)
        //: img.isHidden = true
        img.isHidden = true
    ...
    private lazy var videoCallManager: ParticipantMenuManager = .init()
    //: lazy var editBtn: UIButton = {
    lazy var editBtn: UIButton = {
        //: let btn = UIButton.init(type: .custom)
        let btn = UIButton(type: .custom)
        //: btn.setTitle("Edit".localized, for: .normal)
        btn.setTitle((String(appTopIdent.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(overdoVideo), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
...
extension GrantLiveBottomView {
    ...
    @objc func untilLast() {
        ...
        kResolutionText.atJson(eventID: m_barUrl, toUid: userInfoModel.uid)
        //: if PathAppManager.share.loginUserMode.sex == Gender.male.rawValue {
        if PathAppManager.share.loginUserMode.sex == ConnectionClusterLiteral.male.rawValue { // 男性用户
            //: videoCallManager.checkAndCallVideo()
            videoCallManager.infoPost()
            ...
            videoCallManager.strikeModel(videoPrice: userInfoModel.videoBean, voicePrice: userInfoModel.voiceBean, vipPrompt: userInfoModel.prompt)
        }
    }
    ...
    @objc private func report() {
        //: NamePushManager.share.func__pushToPriveteChatVC(chatID: userInfoModel.uid)
        NamePushManager.share.signature(chatID: userInfoModel.uid)
        //: uploadRecord.uploadRecordEvent(eventID: ClickProfileChatButton, toUid: userInfoModel.uid)
        kResolutionText.atJson(eventID: dream_inviteTitle, toUid: userInfoModel.uid)
    }

    /// edit
    //: @objc private func editButtonClick() {
    @objc private func overdoVideo() {
        //: let vc = TalkingEditProfilesVC()
        let vc = DataProfilesVc()
...
extension GrantLiveBottomView {
    ...
    func refresh(_ userModel: ModelMeasurable) {
        ...
        // 根据角色 展示/隐藏 按钮
        //: if userModel.uid == PathAppManager.share.loginUid {
        if userModel.uid == PathAppManager.share.loginUid { // 本人资料页
            //: editBtn.isHidden = false
            editBtn.isHidden = false
            ...
            return
                    ...
                    //: } else if PathAppManager.share.loginUid != FacultyThen.getCustomerServiceID(),
        } else if PathAppManager.share.loginUid != FacultyThen.withAdd(),
                  //: PathAppManager.share.loginUserMode.sex == userModel.sex,
                  PathAppManager.share.loginUserMode.sex == userModel.sex,
        ...
        if PathAppManager.share.loginUserMode.sex == ConnectionClusterLiteral.male.rawValue {
            ...
            if PathAppManager.share.loginUserMode.loungePlus == false { // 男性非会员
                ...
                if PathAppManager.share.appStatus == AssemblageHashable.normal.rawValue {
                    ...
                    maleVipPriceLab.text = String.twine(videoPrice: userInfoModel.videoVIPPrice)
                }
                //: vipIcon.isHidden = true
                vipIcon.isHidden = true
                //: callStrLab.text = String.getMaleVIPVideoPriceString(videoPrice: userInfoModel.videoPrice)
                callStrLab.text = String.twine(videoPrice: userInfoModel.videoPrice)

                //: } else {
            } else {
                //: let colors = [UIColor(hex: "#AB57F6")!.cgColor, UIColor(hex: "#FC57B7")!.cgColor, UIColor(hex: "#FE9074")!.cgColor]
                let colors = [UIColor(hex: (m_finishData.replacingOccurrences(of: "row", with: "#") + kGestureCenterFormat.replacingOccurrences(of: "input", with: "6")))!.cgColor, UIColor(hex: (String(main_liveName)))!.cgColor, UIColor(hex: (String(main_locationPath.suffix(7))))!.cgColor]
                ...
                videoBtn.setBackgroundImage(UIImage.garrison(colors: colors, size: CGSize(width: actualWidth(w: 250), height: actualWidth(w: 36))), for: .normal)
                ...
                callStrLab.text = String.twine(videoPrice: userInfoModel.videoVIPPrice)
            }
            ...
            callStrLab.text = (String(mainContentPath) + String(showOkContent)).localized
        }
    }
    ...
    private func allow() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(dreamOkTitle)
        }
    }
}
