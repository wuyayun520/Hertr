...
fileprivate let show_maxByFormat:[UInt8] = [0x44,0x43,0x44,0x59,0x5,0x4e,0x42,0x49,0x48,0x5f,0x17,0x4,0xd,0x45,0x4c,0x5e,0xd,0x43,0x42,0x59,0xd,0x4f,0x48,0x48,0x43,0xd,0x44,0x40,0x5d,0x41,0x48,0x40,0x48,0x43,0x59,0x48,0x49]

/*: "Gifts received" :*/
fileprivate let showDayId:String = "Giftsmodel import"
...
class WillViewDelegate: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_maxByFormat.map{$0^45}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLab: UILabel = {
        ...
        lb.text = (String(showDayId.prefix(5)) + " received").localized
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(DiskView.self, forCellWithReuseIdentifier: DiskView.className())
        //: return collect
        return collect
        //: }()
    }()
...
extension WillViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: DiskView.className(), for: indexPath) as! DiskView
        //: let model: TalkingUserReceivedGiftModel = self.giftsDataArray[indexPath.row]
        let model: SomeoneTransformable = self.giftsDataArray[indexPath.row]
        //: cell.configGiftItemCellWithData(giftModel: model, isNewProfile)
        cell.withImage(giftModel: model, isNewProfile)
        //: return cell
        return cell
    }
...
extension WillViewDelegate {
    //: private func setupSubviews() {
    private func makeSubviewsGlobal() {
