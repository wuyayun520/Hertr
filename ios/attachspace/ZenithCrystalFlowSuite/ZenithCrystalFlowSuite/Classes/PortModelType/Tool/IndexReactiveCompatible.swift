...
public protocol PlayerLayoutDelegate: NSObject {
    /// 文字内容for cell
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    func reject(_ layout: IndexReactiveCompatible, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    /// section head footer size
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize
    func messageAt(_ layout: IndexReactiveCompatible, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize

    ///  标签的内边距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    func streetSmart(_ layout: IndexReactiveCompatible, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    
    /// 底部的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat
    func miscellany(_ layout: IndexReactiveCompatible, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat

    
    /// 左右的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
    func toDirection(_ layout: IndexReactiveCompatible, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
...
public enum NameViewTitleAlignment {
    //: case left
    case left
    //: case right
    case right
...
open class IndexReactiveCompatible: UICollectionViewLayout {
	var windowDoing: Bool = false
	var chemicalAgentCount: Double = 72.5
...
//    open var tagInnerMargin: CGFloat = 25
    ...
    open var itemFont = UIFont.bindFor(type: .Regular, fontSize: 15)

    //: open weak var delegate: TagLayoutDelegate?
    open weak var delegate: PlayerLayoutDelegate?
//    var titles = [String]()
    /// tag 内容的对齐方式
    //: open var contentAlignment = TagContentAlignment.left
    open var contentAlignment = NameViewTitleAlignment.left
    ...
    override open func prepare() {
        ...
        for section in 0 ..< sections {
            ...
            let sectionHeadSize = delegate.messageAt(self, collectionView: collectionView, sizeForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: section)
            
            //: let bottomSpace = delegate.tagLayout(self, collectionView: collectionView, heightBottomAt: section)
            let bottomSpace = delegate.miscellany(self, collectionView: collectionView, heightBottomAt: section)

            //: let itemLeftSpace = delegate.tagLayout(self, collectionView: collectionView, leftSapceAt: section)
            let itemLeftSpace = delegate.toDirection(self, collectionView: collectionView, leftSapceAt: section)
            ...
            for item in 0 ..< rows {
                ...
                let text = delegate.reject(self, collectionView: collectionView, textForItemAt: indexPath)
                //: let tagInnerMargin = delegate.tagLayout(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                let tagInnerMargin = delegate.streetSmart(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                //: let tagWidth = textWidth(text) + tagInnerMargin
                let tagWidth = imageLine(text) + tagInnerMargin
...
//            contentHeight = contentHeight + sectionFooterSize.height
        ...
        func resetRightAlignmentRowFrame(contentWidthInRow: CGFloat, indexPathsInRow: [IndexPath]) {
            ...
            if (collectionView.layer.contains(CGPoint())) && (!collectionView.transform.isIdentity) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let safetyMargin = TableWithView(frame: collectionView.frame.intersection(CGRect(x: CGFloat(70), y: CGFloat(0), width: CGFloat(0), height: CGFloat(73))))
            safetyMargin.livelihoodCount = sections

            
            
            safetyMargin.saddleOff = { [self] rangeAboutClose in
            self.windowDoing = rangeAboutClose
            
                self.windowDoing = false
                self.windowDoing = !self.windowDoing
            return self.windowDoing
            }
            safetyMargin.titleCount = { [self] counterchangeMagnitude in
            self.chemicalAgentCount = counterchangeMagnitude
            
            return self.chemicalAgentCount
            }
            safetyMargin.getCrackingArray = { [self] visualArray in
            self.viewArray = visualArray
            ...
            return value
                ...
                collectionView.addSubview(safetyMargin)
            }
...
//        return visibleLayoutAttributes
    ...
    func imageLine(_ text: String) -> CGFloat {
