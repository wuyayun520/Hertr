...
NSString *StringFromCoachData(Byte *data);


//: Width
Byte app_resultValue[] = {46, 5, 13, 180, 101, 161, 89, 188, 153, 51, 126, 2, 229, 104, 116, 100, 105, 87, 41};


//: Height
Byte app_targetText[] = {59, 6, 6, 129, 155, 172, 116, 104, 103, 105, 101, 72, 118};


//: 请打开相机权限
Byte app_windowTrackName[] = {51, 21, 13, 248, 136, 18, 68, 19, 229, 128, 28, 238, 137, 144, 153, 233, 131, 157, 230, 186, 156, 230, 184, 155, 231, 128, 188, 229, 147, 137, 230, 183, 175, 232, 43};
...
@interface DateCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
                         ...
                         frame:(BOOL)needYuvOutput
{
    ...
    if (self) {
        ...
        self.videoDevice = [self withSend:iDevicePosition];
...
- (void)inwardObject:(CGPoint)point result:(CGRect)frame
{
    ...
    [self authorSName:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           video:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                solarisation:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)authorSName:(AVCaptureFocusMode)focusMode video:(AVCaptureExposureMode)exposureMode solarisation:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self add:^(AVCaptureDevice *captureDevice) {
...
- (void)add:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self withSend:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self output]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:StringFromCoachData(app_resultValue)] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:StringFromCoachData(app_targetText)] floatValue];
...
- (CGRect)meetFit:(CGRect)rect take:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:StringFromCoachData(app_resultValue)] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:StringFromCoachData(app_targetText)] floatValue];
...
- (BOOL)output
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:StringFromCoachData(app_windowTrackName)
...
- (AVCaptureDevice *)withSend:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(appellative:ageConnection:gift:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate appellative:captureOutput ageConnection:sampleBuffer gift:connection];
        }
    }
...
Byte * CoachDataToCache(Byte *data) {
    int shoot = data[0];
    int managerData = data[1];
    int add = data[2];
    if (!shoot) return data + add;
    for (int i = 0; i < managerData / 2; i++) {
        int begin = add + i;
        int end = add + managerData - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[add + managerData] = 0;
    return data + add;
}

NSString *StringFromCoachData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)CoachDataToCache(data)];
}  
