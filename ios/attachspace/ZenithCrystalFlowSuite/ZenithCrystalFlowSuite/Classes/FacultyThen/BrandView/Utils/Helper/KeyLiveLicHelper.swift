...
/*: "SENSEME" :*/
fileprivate let user_onData:String = "SrenderNSrenderMrender"

/*: "lic" :*/
fileprivate let main_sharedStr:String = "buttonic"

/*: "mf/Index/senseTime" :*/
fileprivate let user_imageMsg:String = "mf/Indequal tool"
fileprivate let noti_accountFormat:[Character] = ["e","n","s","e","T","i","m","e"]

/*: "url" :*/
fileprivate let mainToStr:[Character] = ["u","r","l"]

/*: "md5" :*/
fileprivate let main_pathValue:[Character] = ["m","d","5"]

/*: "SENSEMELIC" :*/
fileprivate let main_textMarginValue:String = "SaddNSaddMadd"
fileprivate let dream_eyeValue:String = "LIsharp"

/*: "App/ST/" :*/
fileprivate let userByKey:[Character] = ["A","p","p","/","S","T","/"]

/*: "GET" :*/
fileprivate let showTextPlaceMessage:[Character] = ["G","E","T"]
...
@objcMembers public class KeyLiveLicHelper: NSObject {
    //: static public let share = STLicHelper()
    public static let share = KeyLiveLicHelper()
    //: private override init() {}
    override private init() {}

    // 校验license有效性 先本地->后网络
    //: public func checkLicense() -> Bool {
    public func positionLicense() -> Bool {
    ...
    func videoShould() -> Bool {
        //: if let path = PodspecBundle.bundle.path(forResource: "SENSEME", ofType: "lic") {
        if let path = StreamPodspecBundle.bundle.path(forResource: (user_onData.replacingOccurrences(of: "render", with: "E")), ofType: (main_sharedStr.replacingOccurrences(of: "button", with: "l"))) {
            //: let licenseData = NSData(contentsOfFile: path)
            let licenseData = NSData(contentsOfFile: path)
            //: if licenseData != nil {
            if licenseData != nil {
                //: return checkActiveCodeWithData(dataLicense: licenseData!)
                return noseEffect(dataLicense: licenseData!)
            }
        }
    ...
    func watch() -> Bool {
        ...
        if FileManager.default.fileExists(atPath: path as String) {
            ...
            if licData != nil {
                ...
                return noseEffect(dataLicense: licData!)
            }
        }
    ...
    func noseEffect(dataLicense: NSData) -> Bool {
...
extension KeyLiveLicHelper {
    // 获取美颜license
    //: public func checkRemoteLicInfoWith(completion: @escaping Completionblock) {
    public func ofRemoteSearchDetailsWith(completion: @escaping Completionblock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = PostulationReactiveCompatible()
        //: reqModel.requestPath = "mf/Index/senseTime"
        reqModel.requestPath = (String(user_imageMsg.prefix(6)) + "ex/s" + String(noti_accountFormat))
        ...
        show_pointBottomPath.ingathering(model: reqModel) { _, result, _ in
            ...
            let licLink = json[(String(mainToStr))].stringValue
            //: let md5 = json["md5"].stringValue
            let md5 = json[(String(main_pathValue))].stringValue
            //: if self.checkLicFileMd5With(md5: md5 as NSString) == false {
            if self.coequal5(md5: md5 as NSString) == false {
                //: if  licLink.count > 0 {
                if licLink.count > 0 {
                    //: self.downloadLicsenWith(licUrl: licLink as NSString, completion: completion)
                    self.dataSmartSum(licUrl: licLink as NSString, completion: completion)
                    //: return
                    return
                }
            }
    ...
    func coequal5(md5: NSString) -> Bool {
        //: if md5.length <= 0 {
        if md5.length <= 0 {
            //: return false
            return false
        ...
        let fileMd5: NSString = ShooterAnimatTool.shared.serviceView(filePath: filePath as String) as NSString
    ...
    func author() -> NSString {
        //: let stDirectory = licDiskDirectory()
        let stDirectory = contentUserView()
        //: return stDirectory.appendingPathComponent("SENSEMELIC") as NSString
        return stDirectory.appendingPathComponent((main_textMarginValue.replacingOccurrences(of: "add", with: "E") + dream_eyeValue.replacingOccurrences(of: "sharp", with: "C"))) as NSString
    }

    //: func licDiskDirectory() -> NSString {
    func contentUserView() -> NSString {
        ...
        let stDirectory = docDirectory.appendingPathComponent((String(userByKey)))
    ...
    func dataSmartSum(licUrl: NSString, completion: @escaping Completionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (String(showTextPlaceMessage))
        ...
        URLSession.shared.dataTask(with: request as URLRequest) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                if data != nil {
                    ...
                    self.makeConversation(licData: data! as NSData)
    ...
    func makeConversation(licData: NSData) {
        //: let path = licDiskPath()
        let path = author()
        //: let fm = FileManager.default
        let fm = FileManager.default
