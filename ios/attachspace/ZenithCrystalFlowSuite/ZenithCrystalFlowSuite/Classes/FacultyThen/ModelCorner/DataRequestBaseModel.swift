...
/*: "http://stat. :*/
fileprivate let user_indexFormat:String = "gift label text cloudhttp://s"
fileprivate let appSectionIdent:String = "show addtat."

/*: .com" :*/
fileprivate let mMessageTitle:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let user_viewPushValue:String = "top"
fileprivate let dream_labTitle:String = "size manager selfttp://"

/*: "https://" :*/
fileprivate let mainProgressFormat:[Character] = ["h","t","t","p","s",":","/"]
fileprivate let notiLocationUrl:String = "view"

/*: "http://m. :*/
fileprivate let notiUserIdent:[Character] = ["h"]
fileprivate let dreamModelTitle:[Character] = ["t","t","p",":","/","/","m","."]

/*: "http:// :*/
fileprivate let app_acceptStr:String = "http://text scale request"

/*: "-1" :*/
fileprivate let appToolIdent:String = "-1"

/*: "3,90" :*/
fileprivate let notiSucceedIdent:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let kBackgroundKey:[Character] = ["1","9","9","0","-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let kFollowKey:[Character] = ["M","M","-","d","d","-"]
fileprivate let dream_ofMsg:String = "collectioncollectioncollectioncollection"

/*: "id" :*/
fileprivate let appDataFormat:[Character] = ["i","d"]
...
class PostulationReactiveCompatible: NSObject {
    ...
    var requestType: ViewModelRequestType?
    ...
    override init() {
        ...
        self.requestServer = LiveAddrThen.share.serverUrlStr
...
struct SunProtectionFactorModelType: HandyJSON {
    //: var errno: Int!
...
public struct DataErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct AddressMeasurable: HandyJSON {
    ...
    var CSConfig = WithConfigModel() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(user_indexFormat.suffix(8)) + String(appSectionIdent.suffix(4))) + "\(dreamRecordPath)" + (String(mMessageTitle))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((user_viewPushValue.replacingOccurrences(of: "top", with: "h") + String(dream_labTitle.suffix(6)))) && !statDomain.hasPrefix((String(mainProgressFormat) + notiLocationUrl.replacingOccurrences(of: "view", with: "/"))) {
                //: return "http://".appending(statDomain)
                return (user_viewPushValue.replacingOccurrences(of: "top", with: "h") + String(dream_labTitle.suffix(6))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(notiUserIdent) + String(dreamModelTitle)) + "\(dreamRecordPath)" + (String(mMessageTitle))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(app_acceptStr.prefix(7))) + "\(urlDomain)"
        }
    }
...
public class ReasonTransformable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [PriceDirectionHandyJSON] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [PriceDirectionHandyJSON] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [PriceDirectionHandyJSON] = [] // 语音收费设置
    ...
    var vipSetting = [ToMeasurable]() // vip皮肤
    ...
    var tabPageBanner = [FailBannerModel]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = TableModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class RegionHandyJSON: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [VideoTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [VideoTagModel]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [WillModelHandyJSON]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (notiSucceedIdent.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.rivetLineCover(self.birthday ?? (String(kBackgroundKey)), dateFormat: (String(kFollowKey) + dream_ofMsg.replacingOccurrences(of: "collection", with: "y")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.clickWait().year else { return "0" }
...
struct WillModelHandyJSON: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct VideoTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func analogise(compareTo: VideoTagModel) -> Bool {
        //: return
        return
...
struct LabelStyleModelType: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [RawDataHandyJSON]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [RawDataHandyJSON]!
}

//: struct UserTagTypeModel: HandyJSON {
struct RawDataHandyJSON: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [VideoTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct WithConfigModel: HandyJSON {
...
struct TableModelType: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
