...
fileprivate let k_messageColorFalseData:[UInt8] = [0xeb,0xec,0xeb,0xf6,0xaa,0xe1,0xed,0xe6,0xe7,0xf0,0xb8,0xab,0xa2,0xea,0xe3,0xf1,0xa2,0xec,0xed,0xf6,0xa2,0xe0,0xe7,0xe7,0xec,0xa2,0xeb,0xef,0xf2,0xee,0xe7,0xef,0xe7,0xec,0xf6,0xe7,0xe6]

private func userCommit(range num: UInt8) -> UInt8 {
    return num ^ 130
}

/*: "FFD855" :*/
fileprivate let appModelBigIdent:String = "titleD855"
...
class BondReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - main_barText
                ...
                self.sizeX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_messageColorFalseData.map{userCommit(range: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (appModelBigIdent.replacingOccurrences(of: "title", with: "FF")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: showRecordData, y: 0, width: main_barText, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - main_barText, y: 0, width: main_barText, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.futurism(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: showRecordData, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.futurism(0, 0, 0, 0.6)
...
extension BondReactiveCompatible {
    ...
    func sizeX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - main_barText,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - main_barText)
    }
}
...
extension BondReactiveCompatible {
    ...
    @objc func quantityerruptInfo(gesture: UIPanGestureRecognizer) {
    ...
    @objc func targetTo(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension BondReactiveCompatible {
    ...
    private func sizeMake() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(quantityerruptInfo(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(targetTo(gesture:)))
    ...
    func imageXposition(_ xposition: CGFloat) {
