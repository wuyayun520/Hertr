...
fileprivate let kAngleText:[UInt8] = [0x8f,0x88,0x8f,0x92,0xce,0x85,0x89,0x82,0x83,0x94,0xdc,0xcf,0xc6,0x8e,0x87,0x95,0xc6,0x88,0x89,0x92,0xc6,0x84,0x83,0x83,0x88,0xc6,0x8f,0x8b,0x96,0x8a,0x83,0x8b,0x83,0x88,0x92,0x83,0x82]

/*: "#4FAAFF" :*/
fileprivate let kVideoTitle:String = "#4FAAFFlabel if finish type size"

/*: "reply" :*/
fileprivate let m_modelPath:String = "cameraply"

/*: "： :*/
fileprivate let k_statusIdent:[Character] = ["："]

/*: "F5F5F5" :*/
fileprivate let appDataTitle:String = "equalequalequal"
...
class QueryThen: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kAngleText.map{$0^230}, encoding: .utf8)!)
    }
...
extension QueryThen {
    ...
    @objc func everyOutput(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension QueryThen {
    //: func configCell(model: TalkingCommentReplyModel) {
    func commonwealth(model: IndexMeasurable) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(kVideoTitle.prefix(7))))
            ...
            let font = UIFont.indicatorSize(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (m_modelPath.replacingOccurrences(of: "camera", with: "re")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.moreRegard(), UIColor.statusBy(), UIColor.moreRegard(), UIColor.statusBy()]
                ...
                colorA = [UIColor.moreRegard(), UIColor.statusBy()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func followThe(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: app_screenFailMarginFormat - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (appDataTitle.replacingOccurrences(of: "equal", with: "F5")))?.cgColor
...
extension QueryThen {
    ...
    private func setupVisible() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(everyOutput(sender:)))
