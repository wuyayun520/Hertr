...
fileprivate let showBlockKey:[UInt8] = [0xc8,0xcf,0xc8,0xd5,0x89,0xc2,0xce,0xc5,0xc4,0xd3,0x9b,0x88,0x81,0xc9,0xc0,0xd2,0x81,0xcf,0xce,0xd5,0x81,0xc3,0xc4,0xc4,0xcf,0x81,0xc8,0xcc,0xd1,0xcd,0xc4,0xcc,0xc4,0xcf,0xd5,0xc4,0xc5]

private func sizeEqual(effect num: UInt8) -> UInt8 {
    return num ^ 161
}
...
class DiskWithThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showBlockKey.map{sizeEqual(effect: $0)}, encoding: .utf8)!)
    }
    ...
    init(frame: CGRect, sliderWidth: CGFloat, sliderColor: UIColor, progressHeight: CGFloat, progressBgColor: UIColor, progressPlayedColor: UIColor, progressCachedColor: UIColor, isShowCorner: Bool) {
        ...
        self.numberConstraint()
        //: self.bindInteraction()
        self.giveAndTakeName()
    }
    ...
    private lazy var progressView: IndexView = {
        ...
        let v = IndexView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(startingBlock(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(magnitude(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension DiskWithThen {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func startingBlock(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func magnitude(gesture: UIPanGestureRecognizer) {
