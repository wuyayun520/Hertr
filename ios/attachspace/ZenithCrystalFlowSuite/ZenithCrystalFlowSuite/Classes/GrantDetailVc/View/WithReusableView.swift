...
fileprivate let noti_allowDeleteMsg:[UInt8] = [0x98,0x9f,0x98,0x85,0xd9,0x92,0x9e,0x95,0x94,0x83,0xcb,0xd8,0xd1,0x99,0x90,0x82,0xd1,0x9f,0x9e,0x85,0xd1,0x93,0x94,0x94,0x9f,0xd1,0x98,0x9c,0x81,0x9d,0x94,0x9c,0x94,0x9f,0x85,0x94,0x95]
...
class WithReusableView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_allowDeleteMsg.map{$0^241}, encoding: .utf8)!)
    }

    //: func customUI() {
    func motility() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(meanSun))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(searched))
        //: doubletap.numberOfTapsRequired = 2
        doubletap.numberOfTapsRequired = 2
        //: self.addGestureRecognizer(doubletap)
        self.addGestureRecognizer(doubletap)
    
            if (imageView.isMultipleTouchEnabled) && (imageView.layer.isHidden != false) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let enableicialDocument = MarkView(frame: imageView.bounds.integral)


            enableicialDocument.colorNeedDictionary = { [self] principalDictionary in
            self.byDictionary = principalDictionary
            
            guard var value = self.byDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                imageView.addSubview(enableicialDocument)
            }
...
extension WithReusableView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func meanSun(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func searched(tap: UIGestureRecognizer) {
    ...
    func toUrl(url: String) {
        ...
        imageView.glycerolizeNeed(urlStr: url)
    }
