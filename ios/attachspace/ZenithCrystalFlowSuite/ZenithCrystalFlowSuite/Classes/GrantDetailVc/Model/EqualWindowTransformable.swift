...
/*: "id" :*/
fileprivate let k_colorIdent:[Character] = ["i","d"]
...
// __CLOSE_PRINT__
//
//  EqualWindowTransformable.swift
//  AbroadTalking
//
...
struct EqualWindowTransformable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct MomentHandyJSON: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum HeadspringSendable: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct PossibilityHandyJSON: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: ReasonSchemaConvertible?
}

///
//: enum MomentVideoStatus: String {
enum ReasonSchemaConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension EqualWindowTransformable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func sortPost(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(KeySizeView.commentCapacity(text: content!, size: CGSize(width: app_screenFailMarginFormat - 54, height: CGFloat(MAXFLOAT)), font: UIFont.bindFor(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == HeadspringSendable.Photo.rawValue || momentType == HeadspringSendable.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
