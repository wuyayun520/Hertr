...
/*: "id" :*/
fileprivate let mainAtActualUrl:String = "ID"

/*: "reply" :*/
fileprivate let app_toolPath:String = "repobservery"

/*: :  :*/
fileprivate let mainEraseValue:String = ":share"

/*:  : :*/
fileprivate let mainSuiteValue:String = " :"
...
struct VideoMeasurable: HandyJSON {
    //: var cid: String?
...
extension VideoMeasurable {
    //: mutating func caculateCommentItemHeight() {
    mutating func keyScreen() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if PresentReactiveCompatible.shared.direction == .leftToRight {
                ...
                tempContent = (app_toolPath.replacingOccurrences(of: "observer", with: "l")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (app_toolPath.replacingOccurrences(of: "observer", with: "l")).localized
            }
        ...
        let sizeheight = Int(KeySizeView.commentCapacity(text: tempContent, size: CGSize(width: app_screenFailMarginFormat - 70, height: CGFloat(MAXFLOAT)), font: UIFont.bindFor(type: .Regular, fontSize: 16), lineNumber: 0).height)
