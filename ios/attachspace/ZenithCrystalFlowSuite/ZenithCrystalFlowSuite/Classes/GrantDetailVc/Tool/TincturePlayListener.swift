...
/*: "%@%@" :*/
fileprivate let appRemindValue:String = "%load%load"

/*: "/TXCache" :*/
fileprivate let main_dataMessage:String = "/TXCacimage cookie for array"
fileprivate let showOfFormat:String = "hcollection"
...
enum DistrictManagerRenderMode {
    //: case FILL_SCREEN
...
protocol ToImageThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func funcDismissSocialStation(player: TincturePlayListener, status: ModelPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func upTo(player: TincturePlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func lineList(player: TincturePlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class TincturePlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: ToImageThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: ModelPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = TincturePlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(main_dataMessage.prefix(6)) + showOfFormat.replacingOccurrences(of: "collection", with: "e"))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(watchful), userInfo: nil, repeats: true)
...
extension TincturePlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func backstop(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: user_topData.disobey(), with: dreamRecordPath)
    ...
    @objc func watchful() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.lineList(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func transformMode(renderMode: DistrictManagerRenderMode) {
    ...
    func viewMute(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isData == true {
        if PublishDataSocketDelegate.shared.isData == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func tillVoiceClick() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.funcDismissSocialStation(player: self, status: self.playStatus!)
        }
    }
...
extension TincturePlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.upTo(player: self, duration: Int(duration), currentTime: Int(currentTime))
