...
fileprivate let noti_quickKey:[UInt8] = [0x86,0x85,0x89,0x8b,0x9e,0x83,0x85,0x84,0xca,0x8b,0x9f,0x9e,0x82,0x85,0x98,0x83,0x90,0x8b,0x9e,0x83,0x85,0x84,0xb9,0x9e,0x8b,0x9e,0x9f,0x99,0xca,0x83,0x99,0xca]

/*: "get location failed. error: :*/
fileprivate let showPathId:String = "get lgift gift else following color"
fileprivate let k_loadStr:String = "on fself content let"
fileprivate let mNameContent:String = "d. eexplore lab close"
fileprivate let k_countMessage:[Character] = ["r","r","o","r",":"]
...
// __CLOSE_PRINT__
//
//  LocationWithManagerDelegate.swift
//  AbroadTalking
//
...
enum PermissionWindowVersionMirrorPath: Int {
    //: case firstEntry = 0
    case firstEntry = 0
...
class LocationWithManagerDelegate: NSObject {
    ...
    var locationType: PermissionWindowVersionMirrorPath?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: LocationWithManagerDelegate?
    //: class func shared() -> TalkingLocationManager {
    class func tableTag() -> LocationWithManagerDelegate {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = LocationWithManagerDelegate()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func useMobile(block: @escaping Locationblock) {
    ...
    func modeOff(block: @escaping Authblock) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) { [weak self] in
            ...
            self.changeShapeCounteraction()
        }
    }

    /// 设备是否开启了定位服务
    //: func hasLocationService() -> Bool {
    func compareService() -> Bool {
    ...
    func locationWithoutAuthorisation() -> PermissionWindowVersionMirrorPath {
    ...
    func authority() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: noti_quickKey.map{$0^234}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension LocationWithManagerDelegate: CLLocationManagerDelegate {
    ...
    private func changeShapeCounteraction() {
        ...
        switch locationWithoutAuthorisation() {
        //: case .firstEntry:
        case .firstEntry:
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(showPathId.prefix(5)) + "ocati" + String(k_loadStr.prefix(4)) + "aile" + String(mNameContent.prefix(4)) + String(k_countMessage)) + "\(error.localizedDescription)")
    }
}
